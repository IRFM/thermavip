...
public:
	...
	{
		edit->setText(mapFileSystem()->address());
	}
	virtual void apply() 
	{
...
VipMapFileSystemTreeItem::VipMapFileSystemTreeItem(const VipPath & path, VipMapFileSystemTree * tree, bool fake)
	:QTreeWidgetItem(), m_path(path), m_need_full_update(false), m_need_attribute_update(false), m_fake(fake), m_custom_dir(false), m_custom_file(false)
{
	if (!m_fake)
		tree->addItem(this);
...
bool VipMapFileSystemTreeItem::setChildren(const VipPathList & children)
{
	if (isCustom())
		return false;
	...
	if (m_children.toSet() != children.toSet())
	{
	...
	if (item->isExpanded())
	{
		...
		else if (m_need_full_update)
		{
			...
			for (int i = 0; i < tmp.size(); ++i)
			{
				...
				if (tmp[i].isDir()) child = new VipMapFileSystemTreeDirItem(tmp[i], item->tree());
				else child = new VipMapFileSystemTreeItem(tmp[i], item->tree());
				item->addChild(child);
...
VipMapFileSystemTreeDirItem::VipMapFileSystemTreeDirItem(const VipPath & path, VipMapFileSystemTree * tree)
	:QObject(), VipMapFileSystemTreeItem(path, tree) {
...
public:
	...
	VipMapFileSystemTreeUpdate(VipMapFileSystemTree * tree)
		:QThread(), tree(tree), sleepTime(5000), trigger(false)
	{}
...
QTreeWidgetItem * VipMapFileSystemTree::internalItemForPath(QTreeWidgetItem * root, const QStringList & sub_paths, const QString & _prefix)
{
	...
	for (int i = 0; i < sub_paths.size(); ++i)
	{
		if (i == 0)
		{
			if (sub_paths[i].isEmpty()) subpath = "/";
			else subpath = sub_paths[0];
		}
		else if (subpath != "/")
			subpath += "/" + sub_paths[i];
		else
					...
					if (path.canonicalPath().startsWith(child->path().canonicalPath())) {
						...
						if (path.canonicalPath() == child->path().canonicalPath())
							...
							if (QTreeWidgetItem * it =
								internalItemForPath(child, path.canonicalPath().mid(child->path().canonicalPath().size()).split("/", QString::SkipEmptyParts), child->path().canonicalPath()))
								res += it;
						}
					}
				}
...
QTreeWidgetItem * VipMapFileSystemTree::addTopLevelPath(const VipPath & path)
{
	if (m_data->map)
	{
		if (m_data->map->exists(path) || path.isEmpty())
		{
			...
			if (path.isDir()) item = new VipMapFileSystemTreeDirItem(path, this);
			else item = new VipMapFileSystemTreeItem(path, this);
	...
	else if (evt->mimeData()->hasUrls())
	{
		...
		if (it->text(0) == "Shortcuts" && top == it)
		{
			...
			for (int i = 0; i < urls.size(); ++i) {
				...
				lst.append(VipPath(path, QFileInfo(path).isDir()));
			}
			...
			for (int i = 0; i < lst.size(); ++i)
			{
				...
				VipMapFileSystemTreeItem* child = lst[i].isDir() ?
					new VipMapFileSystemTreeDirItem(lst[i], this) :
					new VipMapFileSystemTreeItem(lst[i], this);
				child->setFlags(child->flags() & (~Qt::ItemIsDragEnabled));
				top->addChild(child);

				if (!lst[i].isDir()) child->setAttributes(lst[i].attributes());
	...
	for (int it = 0; it != found.size(); ++it)
	{
		...
		if (item)
		{
			if (!expanded)
			{
				if (item->path().isDir())
					item->setExpanded(false);
				else if (QTreeWidgetItem *it = item->parent())
			...
			else
			{
				...
				{
					QList<QTreeWidgetItem*> found = itemsForPath(items[i], type);
					//for (int j = 0; j < found.size(); ++j) {
					// if (!found[j]) {
					// printf("path : '%s'\n", items[i].canonicalPath().toLatin1().data());
					// found = itemsForPath(items[i], type);
					// break;
					// }
					// }
					for (int j = 0; j < found.size(); ++j)
						found[j]->setExpanded(true);
				}
			}
		}
	}
...
void VipMapFileSystemTree::addItem(QTreeWidgetItem* item)
{
	...
	m_data->items.insertMulti(it->path().canonicalPath(), it);
}
...
static void applyPendingFileSystemSession(VipFileSystemWidget* w, const PendingFileSystemSession& session)
{
	...
	// make unique
	VipPathList shortcuts = QSet<VipPath>::fromList(session.shortcuts).toList();
...
void VipFileSystemWidget::startSearch()
{
	...
	if (m_data->tree->mapFileSystem())
	{
		...
		else if (dirs.size() == 1)
		{
			...
			search.replace("\t", " ");
			QStringList lst = search.split(" ", QString::SkipEmptyParts);
			QList<QRegExp> exps;
			for (int i = 0; i < lst.size(); ++i)
			{
				exps.append(QRegExp(lst[i]));
