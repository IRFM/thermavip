...
class VIP_GUI_EXPORT VipBaseDragWidget : public QFrame, public VipRenderObject
{
	Q_OBJECT
...
public:
	...
	VipBaseDragWidgetMimeData() : QMimeData() {}
};
...
private Q_SLOTS:
	void emitChildChanged(QSplitter* s, QWidget * w, bool added) {
		Q_EMIT childChanged(s, w, added);
	}
...
class VIP_GUI_EXPORT VipViewportArea : public QWidget, public VipRenderObject
{
	Q_OBJECT
public:
	VipViewportArea();
...
public:
	...
	virtual VipMultiDragWidget * createMultiDragWidget() const {
		return new VipMultiDragWidget();
	}

	VipViewportArea * widget() const {
		return qobject_cast<VipViewportArea*>(QScrollArea::widget());
	}
