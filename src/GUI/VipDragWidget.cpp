...
struct VipBaseDragWidget::PrivateData
{
	...
	PrivateData(): mouseButton(Qt::NoButton), destroy(false){}
};



VipBaseDragWidget::VipBaseDragWidget(QWidget * parent )
:QFrame(parent),VipRenderObject(this)
{
...
bool VipBaseDragWidget::isDropable() const
{
	if(!this->testSupportedOperation(Drop) ||
			(this->parentMultiDragWidget() && this->parentMultiDragWidget()->count() == 1 && !this->testSupportedOperation(Drop) ) )
		return false;
	else
	{
...
bool VipBaseDragWidget::isMovable() const
{
	if(!this->testSupportedOperation(Move) ||
			(this->parentMultiDragWidget() && this->parentMultiDragWidget()->count() == 1 && !this->testSupportedOperation(Move) ) )
		return false;
	else
	{
		if(qobject_cast<const VipMultiDragWidget*>(this))
...
bool VipBaseDragWidget::supportMaximize() const
{
	if(!this->testSupportedOperation(Maximize) ||
			(this->parentMultiDragWidget() && this->parentMultiDragWidget()->count() == 1 && !this->testSupportedOperation(Maximize) ) )
		return false;
	else
	{
		if(qobject_cast<const VipMultiDragWidget*>(this))
...
bool VipBaseDragWidget::supportMinimize() const
{
	if(!this->testSupportedOperation(Minimize) ||
			(this->parentMultiDragWidget() && this->parentMultiDragWidget()->count() == 1 && !this->testSupportedOperation(Minimize) ) )
		return false;
	else
	{
		if(qobject_cast<const VipMultiDragWidget*>(this))
...
bool VipBaseDragWidget::supportReceiveDrop() const
{
	if(!this->testSupportedOperation(ReceiveDrop) ||
			(this->parentMultiDragWidget() && this->parentMultiDragWidget()->count() == 1 && !this->testSupportedOperation(ReceiveDrop) ) )
		return false;
	else
	{
		if(qobject_cast<const VipMultiDragWidget*>(this))
...
:QSplitterHandle(orientation,parent), multiDragWidget(multiDragWidget), maxWidth(5)
{
...
VipDragWidgetSplitter::VipDragWidgetSplitter(VipMultiDragWidget * multiDragWidget, Qt::Orientation orientation, QWidget * parent)
	:QSplitter(orientation, parent), multiDragWidget(multiDragWidget), maxWidth(5)
{
	this->setAttribute(Qt::WA_DeleteOnClose);
	this->setAutoFillBackground(true);
}
...
void VipMultiDragWidget::mainResize(int new_size, VipMultiDragWidget::VerticalSide side)
{
	...
	bool maximized = this->isMaximized() ||
		(validTopLevelMultiDragWidget() && validTopLevelMultiDragWidget()->isMaximized());
	QList<int> sizes;
	if (maximized)
