...
public:
	...
	virtual VipNDArray ExtractOneComponent(const VipNDArray & array,const QString & ) const
	{
		return array;
	}
	...
	virtual VipNDArray GetComponent(const QString & ) const
	{
...
public:
	...
	virtual QStringList PixelComponentNames() const{return QStringList()<<"Red"<<"Green"<<"Blue"<< "Alpha";}
...
public:
	...
	virtual QStringList PixelComponentNames() const{return QStringList()<<"Hsl Hue"<<"Hsl Saturation"<<"Hsl Lightness"<<"Hsl Alpha";}

	virtual void SetComponents(const QList<VipNDArray> & components) {
		QList<VipNDArray> tmp;
		for (int i = 0; i < components.size(); ++i) {
			if (i == 0) tmp.append(components[i].toInt32());
			else tmp.append(components[i].toUInt8());
		}
		VipExtractComponents::SetComponents(tmp);
	}
};
...
public:
	...
	virtual QStringList PixelComponentNames() const{return QStringList()<<"Hsv Hue"<<"Hsv Saturation"<<"Hsv Value"<<"Hsv Alpha";}
	virtual void SetComponents(const QList<VipNDArray> & components) {
		QList<VipNDArray> tmp;
		for (int i = 0; i < components.size(); ++i) {
			if (i == 0) tmp.append(components[i].toInt32());
...
public:
	...
	QStringList PixelComponentNames() const{return QStringList()<<"CMYK Cyan"<<"CMYK Magenta"<<"CMYK Yellow"<<"CMYK Black"<<"CMYK Alpha";}
	virtual void SetComponents(const QList<VipNDArray> & components) {
		QList<VipNDArray> tmp;
		for (int i = 0; i < components.size(); ++i) {
			tmp.append(components[i].toUInt8());
...
public:
	...
	virtual QList<QByteArray> InputDataTypes() const{return QList<QByteArray>()<<"complex_f"<<"complex_d";}
	virtual QList<QByteArray> PixelComponentTypes() const {
		QByteArray name = QMetaType::typeName(qMetaTypeId<double>());
		return QList<QByteArray>()<<name<<name;
	}
	virtual QStringList PixelComponentNames() const{return QStringList()<<"Real"<<"Imag";}
	virtual void SetComponents(const QList<VipNDArray> & components) {
		QList<VipNDArray> tmp;
		for (int i = 0; i < components.size(); ++i) {
...
public:
	...
	virtual QStringList PixelComponentNames() const{return QStringList()<<"Amplitude"<<"Argument";}
	virtual void SetComponents(const QList<VipNDArray> & components) {
		QList<VipNDArray> tmp;
		for (int i = 0; i < components.size(); ++i) {
			tmp.append(components[i].toDouble());
...
inline QStringList vipPossibleComponents(const VipNDArray & ar)
{
	...
	if (strcmp(ar.dataName(), "QImage") == 0) {
		return QStringList() << "Color ARGB" << "Color AHSL" << "Color AHSV" << "Color ACMYK";
	}
	else if (strcmp(ar.dataName(), "complex_f") == 0 || strcmp(ar.dataName(), "complex_d") == 0) {
		return QStringList() << "Complex Real/Imag" << "Complex Amplitude/Argument";
	}
	else
		return QStringList();
}
