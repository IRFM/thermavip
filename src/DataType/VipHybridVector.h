...
public:
	...
	VipHybridVector() noexcept : m_size(0) {}
	VipHybridVector(int size)noexcept : m_size(size) {}
	VipHybridVector(int size, const T & elem)noexcept : m_size(size) { fill(elem); }
	...
	const_iterator  end() const noexcept { return m_elems + m_size; }//return elems.end(); }

#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Warray-bounds"
#endif
															// operator[]
	reference operator[](size_type i) noexcept { return m_elems[i]; }
...
#pragma GCC diagnostic pop
	...
	reference at(size_type i) noexcept { return m_elems[i]; }
	const_reference at(size_type i) const noexcept { return m_elems[i]; }

	// front() and back()
	reference front() noexcept { return m_elems[0]; }
	const_reference front() const noexcept { return m_elems[0]; }

	reference back() noexcept { return m_elems[m_size - 1]; }
	const_reference back() const noexcept { return m_elems[m_size - 1]; }

	reference first() noexcept { return front(); }
	const_reference first() const noexcept { return front(); }
	reference last() noexcept { return back(); }
	const_reference last() const noexcept { return back(); }

	// size is constant
	size_type size() const noexcept { return m_size; }// static_cast<size_type>(elems.size()); }
	...
	// direct access to data (read-only)
	const T* data() const noexcept { return &m_elems[0]; }
	T* data() noexcept { return &m_elems[0]; }
...
VipHybridVector<int, Vip::None> operator+(const VipHybridVector<int, N1> & v1, const VipHybridVector<int, N2> & v2)noexcept
{
	...
	{
		tmp = v1;
		for (int i = 0; i < v2.size(); ++i) tmp[i] += v2[i];
	}
	else
	{
		tmp = v2;
		for (int i = 0; i < v1.size(); ++i) tmp[i] += v1[i];
	}
	return tmp;
}
...
VipHybridVector<int, N> vipVector(const VipHybridVector<int, N2> & v) noexcept { return v; }
///Create a dynamic VipHybridVector from a QVector
template < class T> VipHybridVector<T, Vip::None> vipVector(const QVector<T> & v) noexcept { VipHybridVector<T, Vip::None> res; res = v; return res; }
///Create a static VipHybridVector of size 1
template < class T> VipHybridVector<T, 1> vipVector(T c0) noexcept { VipHybridVector<T, 1> res = { { c0 } }; return res; }
///Create a static VipHybridVector of size 2
template < class T> VipHybridVector<T, 2> vipVector(T c0, T c1) noexcept { VipHybridVector<T, 2> res = { { c0,c1 } }; return res; }
///Create a static VipHybridVector of size 3
template < class T> VipHybridVector<T, 3> vipVector(T c0, T c1, T c2) noexcept { VipHybridVector<T, 3> res = { { c0,c1,c2 } }; return res; }
///Create a static VipHybridVector of size 4
template < class T> VipHybridVector<T, 4> vipVector(T c0, T c1, T c2, T c3) noexcept { VipHybridVector<T, 4> res = { { c0,c1,c2,c3 } }; return res; }
///Create a static VipHybridVector of size 5
template < class T> VipHybridVector<T, 5> vipVector(T c0, T c1, T c2, T c3, T c4) noexcept { VipHybridVector<T, 5> res = { { c0,c1,c2,c3,c4 } }; return res; }
///Create a static VipHybridVector of size 6
template < class T> VipHybridVector<T, 6> vipVector(T c0, T c1, T c2, T c3, T c4, T c5) noexcept { VipHybridVector<T, 6> res = { { c0,c1,c2,c3,c4,c5 } }; return res; }
///Create a static VipHybridVector of size 7
template < class T> VipHybridVector<T, 7> vipVector(T c0, T c1, T c2, T c3, T c4, T c5, T c6)noexcept { VipHybridVector<T, 7> res = { { c0,c1,c2,c3,c4,c5,c6 } }; return res; }
///Create a static VipHybridVector of size 8
template < class T> VipHybridVector<T, 8> vipVector(T c0, T c1, T c2, T c3, T c4, T c5, T c6, T c7) noexcept { VipHybridVector<T, 8> res = { { c0,c1,c2,c3,c4,c5,c6,c7 } }; return res; }
///Create a static VipHybridVector of size 9
template < class T> VipHybridVector<T, 9> vipVector(T c0, T c1, T c2, T c3, T c4, T c5, T c6, T c7, T c8)noexcept { VipHybridVector<T, 9> res = { { c0,c1,c2,c3,c4,c5,c6,c7,c8 } }; return res; }
///Create a static VipHybridVector of size 10
template < class T> VipHybridVector<T, 10> vipVector(T c0, T c1, T c2, T c3, T c4, T c5, T c6, T c7, T c8, T c9) noexcept { VipHybridVector<T, 10> res = { { c0,c1,c2,c3,c4,c5,c6,c7,c8,c9 } }; return res; }


#include <tuple>
/// https://stackoverflow.com/questions/2124339/c-preprocessor-va-args-number-of-arguments
/// MACRO version of vipVector, much faster on msvc (?)
#define vip_vector(...) VipHybridVector<int, std::tuple_size<decltype(std::make_tuple(__VA_ARGS__))>::value >{__VA_ARGS__}
