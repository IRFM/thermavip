...
namespace detail
{
	...
	struct ToQStringTransform
	{
		...
		static QString apply(const INTYPE & d) { return QString::number(d); }
		...
		template< class T>
		static QString apply(const std::complex<T> & d) { return typeToString<std::complex<T> >(d); }
		static QString apply(const VipRGB & c) { return typeToString<VipRGB >(c); }

		template< class T>
		QString operator()(const T & src) const { return apply(src); }
	};
	...
	{
		template< class INTYPE>
		static QByteArray apply(const INTYPE & d) { return QByteArray::number(d); }
		...
		template< class T>
		static QByteArray apply(const std::complex<T> & d) { return typeToByteArray<std::complex<T> >(d); }
		static QByteArray apply(const VipRGB & c) { return typeToByteArray<VipRGB >(c); }

		template<class T>
		QByteArray operator()(const T & src) const { return apply(src); }
	};
	...
	{
		template< class T>
		static Res apply(const T & d) { return  static_cast<Res>(d); }
		static Res apply(const QString & d) {
			if (std::is_same<Res, float>::value) return d.toFloat();
			else if (std::is_same<Res, double>::value) return d.toDouble();
			else if (std::is_same<Res, long double>::value) return vipLongDoubleFromString(d);
			else if (std::is_same<Res, bool>::value) { QString str = d.toLower(); return !(str == QString("0") || str == QString("false") || str.isEmpty()); }
			return d.toLongLong();
		}
		static Res apply(const QByteArray & d) {
			if (std::is_same<Res, float>::value) return d.toFloat();
			else if (std::is_same<Res, double>::value) return d.toDouble();
			else if (std::is_same<Res, long double>::value) return vipLongDoubleFromByteArray(d);
			else if (std::is_same<Res, bool>::value) { QString str = d.toLower(); return !(str == QString("0") || str == QString("false") || str.isEmpty()); }
			return d.toLongLong();
		}
		template< class T>
		Res operator()(const T & src) const { return apply(src); }
	};
	...
	{
		template< class T>
		static complex_f apply(const T & d) { return static_cast<complex_f>(d); }
		...
		template< class T>
		complex_f operator()(const T & src) const { return apply(src); }
	};
	...
	{
		template< class T>
		static complex_d apply(const T & d) { return static_cast<complex_d>(d); }
		...
		template< class T>
		complex_d operator()(const T & src) const { return apply(src); }
	};
	...
	struct ToRGB
	{
		...
		template<class T>
		static VipRGB apply(const T &) { return VipRGB(); }
		template<class T>
		VipRGB operator()(const T & src)const { return apply(src); }
	};
	...
	struct Converter { static Out apply(const In & v) { return v; } };
	template< > struct Converter<complex_f, complex_d> { static complex_d apply(const complex_f & v) { return complex_d(v.real(), v.imag()); } };
	template< > struct Converter<complex_d, complex_f> { static complex_f apply(const complex_d & v) { return complex_f(v.real(), v.imag()); } };
	template<class Out > struct Converter<complex_d, Out> { static Out apply(const complex_d & v) { return v.real(); } };
	template<class Out > struct Converter<complex_f, Out> { static Out apply(const complex_f & v) { return v.real(); } };
	...
	struct GetConverterFunction
	{
		...
		static cast get(int data_type) {
			if (data_type == QMetaType::Char) return convertVoid<char, To>;
			else if (data_type == QMetaType::SChar)  return convertVoid<qint8, To>;
			else if (data_type == QMetaType::UChar)  return convertVoid<quint8, To>;
			else if (data_type == QMetaType::Short)  return convertVoid<qint16, To>;
			else if (data_type == QMetaType::UShort)  return convertVoid<quint16, To>;
			else if (data_type == QMetaType::Int)  return convertVoid<qint32, To>;
			else if (data_type == QMetaType::UInt)  return convertVoid<quint32, To>;
			else if (data_type == QMetaType::Long)  return convertVoid<long, To>;
			else if (data_type == QMetaType::ULong)  return convertVoid<unsigned long, To>;
			else if (data_type == QMetaType::LongLong)  return convertVoid<qint64, To>;
			else if (data_type == QMetaType::ULongLong)  return convertVoid<quint64, To>;
			else if (data_type == QMetaType::Float)  return convertVoid<float, To>;
			else if (data_type == QMetaType::Double)  return convertVoid<double, To>;
			else if (data_type == qMetaTypeId<long double>())  return convertVoid<long double, To>;
			else if (data_type == qMetaTypeId<complex_f>())  return convertVoid<complex_f, To>;
			else if (data_type == qMetaTypeId<complex_d>())  return convertVoid<complex_d, To>;
			else return genericConverterVoid<To>;
		}
	};
	...
	VIP_DATA_TYPE_EXPORT bool convert(const void* i_data, uint i_type, const VipNDArrayShape & i_shape, const VipNDArrayShape & i_strides,
