...
namespace detail
{
	...
	struct Conversion
	{
		...
		bool operator==(const Conversion & other) const {
			return source == other.source && dest_type == other.dest_type;
		}
	};
	//to store in QSet
	...
	struct FromValue {
		template <class U>
		static const T& value(const U & v) { return v; }
	...
	struct ConstValue : BaseExpression
	{
		...
		const T value;
		ConstValue() :value(T()) {}
		ConstValue(const ConstValue & other) : value(other.value) {}
		ConstValue(const T & value) :value(FromValue<T>::value(value)) {}
		template< class U>
		ConstValue(const ConstValue<U> & other) : value(FromValue<T>::value(other.value)) {}
		template< class U>
		ConstValue(const U & other) : value(other) {}
		int dataType() const { return qMetaTypeId<T>(); }
		bool isEmpty() const { return false; }
		bool isUnstrided() const { return true; }
		const VipNDArrayShape & shape() const { static const VipNDArrayShape null_shape; return null_shape; }
		template< class ShapeType>
		value_type operator()(const ShapeType &) const { return value; }
	...
	struct ConstValue<QVariant> : BaseExpression
	{
		...
		ConstValue() {}
		template< class T>
		ConstValue(const T & value) :value(QVariant::fromValue(value)) {}
		int dataType() const { return value.userType(); }
		bool isEmpty() const { return value.isNull(); }
		bool isUnstrided() const { return true; }
		const VipNDArrayShape & shape() const { static const VipNDArrayShape null_shape; return null_shape; }
		template< class ShapeType>
		const QVariant & operator()(const ShapeType &) const { return value; }
	...
	/// Get object shape
	template<class Obj>
	const VipNDArrayShape& GetShape(const Obj& obj) noexcept { return obj.shape(); }
	template<class T, int Dims>
	const VipNDArrayShape& GetShape(const VipNDArrayType<T, Dims>& obj) noexcept { return obj.VipNDArray::shape(); }
	template<class T, int Dims>
	const VipNDArrayShape& GetShape(const VipNDArrayTypeView<T, Dims>& obj) noexcept { return obj.VipNDArray::shape(); }
	...
	struct ArrayWrapper : BaseExpression
	{
		...
		ArrayWrapper() {}
		ArrayWrapper(const Array & ar) : array(ar), ptr(ar.ptr()) {}
		...
		template< class ShapeType> const value_type & operator()(const ShapeType & pos) const { return array(pos); }
	...
	struct ArrayWrapper<VipNDArrayType<T, NDims> > : BaseExpression
	{
		...
		ArrayWrapper(const ArrayWrapper<Other> & other)
			: array(FunctorContext::instance().convert<VipNDArrayType<T,NDims> >(other.array)), ptr(array.ptr())
		{
		}
		ArrayWrapper(const ArrayWrapper & other)
			://_shape(other._shape), strides(other.strides), _dataType(other._dataType), ptr(other.ptr)
 array(other.array), ptr(other.ptr)
		{
		}
		ArrayWrapper(const VipNDArrayType<T,NDims> & array) : array(array), ptr(array.ptr()) {} //{
			// _shape = array.shape();
		...
		// }
		template<int ONDims>
		ArrayWrapper(const VipNDArrayType<T, ONDims>& array) : array(array), ptr(array.ptr()) {}
		int dataType() const {
		...
		template< class ShapeType> const value_type & operator()(const ShapeType & pos) const { return ptr[Offset<ShapeType::static_size>::compute(array.strides(), pos)]; }
		value_type operator[](int i) const { return ptr[i]; }
	};
	...
	struct rebind<T, VipNDArrayType<U,NDims>, true> {
		...
		static type cast(const VipNDArrayType<U,NDims> & a) {
			return a;
		}
	};
	...
	struct is_valid_functor : std::is_base_of<VipNDArray, T> { static void apply() {} };
	//template< class T>
	...
	template< class T>
	struct is_valid_functor<ConstValue<T> > : std::true_type { static void apply() {} };
	template< class T>
	struct is_valid_functor<ArrayWrapper<T> > : std::true_type { static void apply() {} };
	...
	struct HasNullType
	{
		...
		static const bool value = std::is_same<t1, NullType>::value ||
			std::is_same<t2, NullType>::value ||
			std::is_same<t3, NullType>::value;
	};
	/// Check if at least one template argument is a NullOperand
	template<class A1, class A2 = void, class A3 = void>
	struct HasArrayType
	{
		static const bool value = std::is_base_of<NullOperand, A1>::value ||
			std::is_base_of<NullOperand, A2>::value ||
			std::is_base_of<NullOperand, A3>::value;
	};
	...
	struct BaseOperator1 : BaseExpression
	{
		...
		BaseOperator1() {}
		BaseOperator1(const ArrayType & op1) : array1(op1) {}
		//define dummy access operator to avoid compilation error
		template< class ShapeType> value_type operator()(const ShapeType &) const { return value_type(); }
...
#define _ENSURE_OPERATOR1_DEF( ... ) \
	...
	struct BaseOperator2 : BaseExpression
	{
		...
		int dataType() const {
			if (_dataType == 0) {
				if (std::is_same<T, NullType>::value) {
					...
					if (dtype == 0) dtype = array1.dataType();
		...
		BaseOperator2() {}
		BaseOperator2(const ArrayType1 & op1, const ArrayType2 & op2) : array1(op1), array2(op2), sh(&_null_shape), _dataType(0) {
			if (!array1.shape().isEmpty() && !array2.shape().isEmpty()) {
				if (array1.shape() == array2.shape())
					sh = &GetShape(array1);
			}
			else {
				if (!array1.shape().isEmpty()) sh = &GetShape(array1);
				else sh = &GetShape(array2);
			}
		}
		//define dummy access operator to avoid compilation error
		template< class ShapeType> value_type operator()(const ShapeType &) const { return value_type(); }
...
#define _ENSURE_OPERATOR2_DEF( ... ) \
	...
	struct BaseOperator3 : BaseExpression
	{
		...
		BaseOperator3() {}
		BaseOperator3(const ArrayType1 & op1, const ArrayType2 & op2, const ArrayType3 & op3) : array1(op1), array2(op2), array3(op3), sh(&_null_shape), _dataType(0) {
...
#define _REBIND_HELPER3(functor) \
	...
	struct rebind<T, functor<A1, A2, A3, hasNullType>, false> {\
		...
		typedef functor<typename rebind<T, typename op::array_type1>::type, typename rebind<T, typename op::array_type2>::type,typename rebind<T, typename op::array_type3>::type, false> type;\
		static type cast(const op & a) { return type(rebind<T, typename op::array_type1>::cast(a.array1), rebind<T, typename op::array_type2>::cast(a.array2), rebind<T, typename op::array_type3>::cast(a.array3)); }\
	};
	...
	namespace cast
	{
		...
		template<class T>
		const T& invariant_cast(const T& v) { return v; }
		inline const unsigned char invariant_cast(const bool v) { return (unsigned char)v; }
	}
...
#define _DECLARE_FUNCTOR_FOR_FUNCTION3(Functor, Function)                                                                                                                                              \
	...
	struct DeduceOperatorType##Functor                                                                                                                                                             \
	{                                                                                                                                                                                              \
		static constexpr bool valid = Support##Functor<typename DeduceArrayType<A>::value_type, typename DeduceArrayType<B>::value_type, typename DeduceArrayType<C>::value_type>::value;      \
		using value_type =                                                                                                                                                                     \
		  typename _DeduceOperatorType##Functor<typename DeduceArrayType<A>::value_type, typename DeduceArrayType<B>::value_type, typename DeduceArrayType<C>::value_type>::value_type;        \
	};                                                                                                                                                                                             \
                                                                                                                                                                                                       \
...
namespace detail {
	...
	{
		ReverseBits() {}
		ReverseBits(const A1 & op1) : BaseOperator1<typename DeduceArrayType<A1>::value_type, A1>(op1) {}
		template< class Coord> typename DeduceArrayType<A1>::value_type operator()(const Coord & pos) const { return ~this->array1(pos); }
	...
	{
		ReverseBits() {}
		ReverseBits(const A1 & op1) : BaseOperator1<NullType, A1>(op1) {}
	};
...
namespace detail {
	...
	{
		Not() {}
		Not(const A1 & op1) : BaseOperator1<typename DeduceArrayType<A1>::value_type, A1>(op1) {}
		template< class Coord> typename DeduceArrayType<A1>::value_type operator()(const Coord & pos) const { return !this->array1(pos); }
	...
	{
		Not() {}
		Not(const A1 & op1) : BaseOperator1<NullType, A1>(op1) {}
	};
	_REBIND_HELPER1(Not)
		template<class A1>
...
namespace detail
{
	...
	struct CastOp : public BaseOperator1<T, A1>
	{
		...
		CastOp() {}
		CastOp(const A1 & op1) : BaseOperator1<T, A1>(op1) {}
		template< class Coord> T operator()(const Coord & pos) const { return  cast::cast(this->array1(pos)); }
...
template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, T>::type vipReal(const T &v) { return v; }
template< class T>
T vipReal(const std::complex<T> & c) { return c.real(); }
VIP_CREATE_FUNCTION1(RealFun, vipReal)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, T>::type vipImag(const T &v) { return (T)0; }
template< class T>
T vipImag(const std::complex<T> & c) { return c.imag(); }
VIP_CREATE_FUNCTION1(ImagFun, vipImag)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, T>::type vipArg(const T &v) { return 0; }
template< class T>
T vipArg(const std::complex<T> & c) { return std::arg(c); }
VIP_CREATE_FUNCTION1(ArgFun, vipArg)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, T>::type vipNorm(const T &v) { return std::norm(std::complex<T>(v)); }
template< class T>
T vipNorm(const std::complex<T> & c) { return std::norm(c); }
VIP_CREATE_FUNCTION1(NormFun, vipNorm)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, T>::type vipConjugate(const T &v) { return v; }
template< class T>
T vipConjugate(const std::complex<T> & c) { return std::complex<T>(c.real(), -c.imag()); }
...
namespace detail {
	...
	template< class A1, class A2, class A3, bool hasNullType = HasNullType<A1, A2, A3>::value>
	struct WhereFun : BaseOperator3<decltype(vipWhere(typename DeduceArrayType<A1>::value_type(), typename DeduceArrayType<A2>::value_type(), typename DeduceArrayType<A3>::value_type())), A1, A2, A3>
	{
		typedef BaseOperator3<decltype(vipWhere(typename DeduceArrayType<A1>::value_type(), typename DeduceArrayType<A2>::value_type(), typename DeduceArrayType<A3>::value_type())), A1, A2, A3> base_type;
		WhereFun() {}
		WhereFun(const A1& op1, const A2& op2, const A3& op3) : base_type(op1, op2, op3) {}
		template< class Coord> typename base_type::value_type operator()(const Coord& pos) const { return (this->array1(pos) ? this->array2(pos): this->array3(pos)); }
	...
	template<class A1, class A2, class A3>
	struct is_valid_functor<WhereFun<A1,A2,A3,false> > : is_valid_op3<A1,A2,A3,try_WhereFun>{};//, std::void_t<decltype(vipWhere(DeduceArrayType<A1>::value_type(),DeduceArrayType<A2>::value_type()))> > :std::true_type{};
}
template<class T1, class T2, class T3>
...
template<class T>
inline typename std::enable_if<std::is_arithmetic<T>::value, std::complex<T> >::type vipMakeComplex(const T &real, const T & imag) { return std::complex<T>(real, imag); }
VIP_CREATE_FUNCTION2(MakeComplexFun, vipMakeComplex)

inline complex_d vipMakeComplexd(double real, double imag) { return complex_d(real, imag); }
VIP_CREATE_FUNCTION2(MakeComplexdFun, vipMakeComplexd)

template<class T>
inline typename std::enable_if<std::is_arithmetic<T>::value, std::complex<T> >::type vipMakeComplexPolar(const T &mag, const T & phase) { return std::polar(mag, phase); }
VIP_CREATE_FUNCTION2(MakeComplexPolarFun, vipMakeComplexPolar)

inline complex_d vipMakeComplexPolard(double mag, double phase) { return std::polar(mag, phase); }
...
template<class T>
inline T vipRed(const VipRgb<T> & rgb) { return rgb.r; }
VIP_CREATE_FUNCTION1(RedFun, vipRed)

template<class T>
inline T vipGreen(const VipRgb<T> & rgb) { return rgb.g; }
VIP_CREATE_FUNCTION1(GreenFun, vipGreen)

template<class T>
inline T vipBlue(const VipRgb<T> & rgb) { return rgb.b; }
VIP_CREATE_FUNCTION1(BlueFun, vipBlue)

template<class T>
inline T vipAlpha(const VipRgb<T> & rgb) { return rgb.a; }
VIP_CREATE_FUNCTION1(AlphaFun, vipAlpha)



template<class T>
inline VipRgb<T> vipSetRed(const VipRgb<T> & rgb, T r) { return VipRgb<T>(r,rgb.g,rgb.b,rgb.a); }
VIP_CREATE_FUNCTION2(SetRedFun, vipSetRed)

template<class T>
inline VipRgb<T> vipSetGreen(const VipRgb<T> & rgb, T g) { return VipRgb<T>(rgb.r, g, rgb.b, rgb.a); }
VIP_CREATE_FUNCTION2(SetGreenFun, vipSetGreen)

template<class T>
inline VipRgb<T> vipSetBlue(const VipRgb<T> & rgb, T b) { return VipRgb<T>(rgb.r, rgb.g, b, rgb.a); }
VIP_CREATE_FUNCTION2(SetBlueFun, vipSetBlue)

template<class T>
inline VipRgb<T> vipSetAlpha(const VipRgb<T> & rgb, T a) { return VipRgb<T>(rgb.r, rgb.g, rgb.b, a); }
...
inline VipRGB vipMakeRGB(quint8 r, quint8 g, quint8 b) { return  VipRGB(r, g, b);}
VIP_CREATE_FUNCTION3(MakeRGBFun, vipMakeRGB)
...
template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipSin(const T &v) { return std::sin(v); }
VIP_CREATE_FUNCTION1(SinFun, vipSin)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipTan(const T &v) { return std::tan(v); }
VIP_CREATE_FUNCTION1(TanFun, vipTan)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipACos(const T &v) { return std::acos(v); }
VIP_CREATE_FUNCTION1(ACosFun, vipACos)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipASin(const T &v) { return std::asin(v); }
VIP_CREATE_FUNCTION1(ASinFun, vipASin)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipATan(const T &v) { return std::atan(v); }
VIP_CREATE_FUNCTION1(ATanFun, vipATan)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipATan2(const T &v1, const T & v2) { return std::atan2(v1, v2); }
VIP_CREATE_FUNCTION2(ATan2Fun, vipATan2)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipCosh(const T &v) { return std::cosh(v); }
VIP_CREATE_FUNCTION1(CoshFun, vipCosh)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipSinh(const T &v) { return std::sinh(v); }
VIP_CREATE_FUNCTION1(SinhFun, vipSinh)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipTanh(const T &v) { return std::tanh(v); }
VIP_CREATE_FUNCTION1(TanhFun, vipTanh)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipACosh(const T &v) { return std::acosh(v); }
VIP_CREATE_FUNCTION1(ACoshFun, vipACosh)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipASinh(const T &v) { return std::asinh(v); }
VIP_CREATE_FUNCTION1(ASinhFun, vipASinh)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipATanh(const T &v) { return std::atanh(v); }
VIP_CREATE_FUNCTION1(ATanhFun, vipATanh)



template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipExp(const T &v) { return std::exp(v); }
VIP_CREATE_FUNCTION1(ExpFun, vipExp)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipSignificand(const T &v) { int exp; return std::frexp(v, &exp); }
VIP_CREATE_FUNCTION1(SignificandFun, vipSignificand)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, int>::type vipExponent(const T &v) { int exp; std::frexp(v, &exp); return exp; }
VIP_CREATE_FUNCTION1(ExponentFun, vipExponent)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipLdexp(const T &v, int exp) { return std::ldexp(v, exp); }
VIP_CREATE_FUNCTION2(LdexpFun, vipLdexp)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipLog(const T &v) { return std::log(v); }
VIP_CREATE_FUNCTION1(LogFun, vipLog)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipLog10(const T &v) { return std::log10(v); }
...
template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipExp2(const T &v) { return std::exp2(v); }
VIP_CREATE_FUNCTION1(Exp2Fun, vipExp2)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipExpm1(const T &v) { return std::expm1(v); }
VIP_CREATE_FUNCTION1(Expm1Fun, vipExpm1)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, int>::type vipIlogb(const T &v) { return std::ilogb(v); }
VIP_CREATE_FUNCTION1(IlogbFun, vipIlogb)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipLog1p(const T &v) { return std::log1p(v); }
VIP_CREATE_FUNCTION1(Log1pFun, vipLog1p)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipLog2(const T &v) { return std::log2(v); }
VIP_CREATE_FUNCTION1(Log2Fun, vipLog2)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipLogb(const T &v) { return std::logb(v); }
VIP_CREATE_FUNCTION1(LogbFun, vipLogb)

template<class T, class T2>
typename std::enable_if<std::is_arithmetic<T>::value && std::is_arithmetic<T2>::value, _DOUBLE>::type vipPow(const T &v, const T2 & p) { return std::pow(v, p); }
VIP_CREATE_FUNCTION2(PowFun, vipPow)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipSqrt(const T &v) { return std::sqrt(v); }
VIP_CREATE_FUNCTION1(SqrtFun, vipSqrt)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipCbrt(const T &v) { return std::cbrt(v); }
VIP_CREATE_FUNCTION1(CbrtFun, vipCbrt)


template<class T, class T2>
typename std::enable_if<std::is_arithmetic<T>::value && std::is_arithmetic<T2>::value, _DOUBLE>::type vipHypot(const T &v, const T2 & p) { return std::hypot(v, p); }
VIP_CREATE_FUNCTION1(HypotFun, vipHypot)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipErf(const T &v) { return std::erf(v); }
VIP_CREATE_FUNCTION1(ErfFun, vipErf)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipErfc(const T &v) { return std::erfc(v); }
VIP_CREATE_FUNCTION1(ErfcFun, vipErfc)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipTGamma(const T &v) { return std::tgamma(v); }
VIP_CREATE_FUNCTION1(TGammaFun, vipTGamma)

template<class T>
typename std::enable_if<std::is_arithmetic<T>::value, _DOUBLE>::type vipLGamma(const T &v) { return std::lgamma(v); }
