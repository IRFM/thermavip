...
struct VIP_DATA_TYPE_EXPORT VipNDArrayHandle : public QSharedData
{
	...
	virtual bool resize(const VipNDArrayShape & start, const VipNDArrayShape & shape, VipNDArrayHandle * dst, Vip::InterpolationType, const VipNDArrayShape & out_start, const VipNDArrayShape & out_shape) const = 0;
	...
	virtual bool exportData(const VipNDArrayShape & this_start, const VipNDArrayShape & this_shape, VipNDArrayHandle * dst, const VipNDArrayShape & dst_start, const VipNDArrayShape & dst_shape) const = 0;
	...
	/// \return true on success, false otherwise
	virtual bool importData(const VipNDArrayShape & this_start, const VipNDArrayShape & this_shape, const VipNDArrayHandle * src, const VipNDArrayShape & src_shape, const VipNDArrayShape & src_start) = 0;
...
namespace detail
{
	...
	public:
		...
		VIP_ALWAYS_INLINE void detach() { if (d->ref.load() != 1) detach_helper(); }
#else
		VIP_ALWAYS_INLINE void detach() { if (d->ref.loadRelaxed() != 1) detach_helper(); }
#endif
		VIP_ALWAYS_INLINE U &operator*() { detach(); return *d; }
		VIP_ALWAYS_INLINE const U &operator*() const noexcept { return *d; }
		VIP_ALWAYS_INLINE U *operator->() { detach(); return d; }
		VIP_ALWAYS_INLINE const U *operator->() const noexcept { return d; }
		VIP_ALWAYS_INLINE operator U *() { detach(); return d; }
		VIP_ALWAYS_INLINE operator const U *() const noexcept { return d; }
		VIP_ALWAYS_INLINE U *data() { detach(); return d; }
		...
		template< class T>
		explicit SharedDataPointer(T *data)noexcept : d(data) { if (d) d->ref.ref(); }
		inline SharedDataPointer(const SharedDataPointer &o)noexcept : d(o.d) { if (d) d->ref.ref(); }
		inline SharedDataPointer(SharedDataPointer&& o)noexcept : d(o.d) { o.d = vipNullHandlePtr(); }
...
namespace detail
{
	...
	struct Istream<char> {
		...
		char operator()(const char &) const {
			qint8 tmp; (*stream) >> tmp; return tmp;
		}
	};
	template<>
	struct Istream<long> {
	...
	struct Istream<unsigned long> {
		...
		unsigned long operator()(const unsigned long &) const {
			quint32 tmp; (*stream) >> tmp; return tmp;
		}
	};
	...
	struct Istream<long double> {
		...
		long double operator()(const long double &) const {
			return vipReadLELongDouble(LD_support, *stream);
		}
	};
	...
	struct StdHandle : public VipNDArrayHandle
	{
		...
		virtual StdHandle* copy() const {
			return new StdHandle(*this);
		}

		virtual void * dataPointer(const VipNDArrayShape & pos) const {
			return pos.isEmpty() ? (T*)opaque : (T*)opaque + vipFlatOffset<false>(strides,pos);
		}
		...
		virtual void * opaqueForPos(void * op, const VipNDArrayShape & pos) const {
			return static_cast<T*>(op) + vipFlatOffset<false>(strides, pos);
		}
		...
		{
			return vipCanConvertStdTypes(qMetaTypeId<T>(), data_type) || vipCanConvert(qMetaTypeId<T>(), data_type);
		}
		virtual bool canImport(int data_type) const
		{
			return vipCanConvertStdTypes(data_type, qMetaTypeId<T>()) || vipCanConvert(data_type, qMetaTypeId<T>());
		}
		...
		virtual bool exportData(const VipNDArrayShape & this_start, const VipNDArrayShape & this_shape, VipNDArrayHandle * dst, const VipNDArrayShape & dst_start, const VipNDArrayShape & dst_shape) const
		{
			...
			{
				return vipArrayTransform(static_cast<const T*>(opaque) + vipFlatOffset<false>(strides, this_start), this_shape, strides,
					static_cast<T*>(dst->opaque) + vipFlatOffset<false>(dst->strides, dst_start), dst_shape, dst->strides, VipNullTransform());
			}

			if (vipCanConvertStdTypes(this_type, other_type)) //conversion of "standard" types (numeric, QString, QByteArray, complex)
				return convert(static_cast<const T*>(opaque) + vipFlatOffset<false>(strides, this_start), this_type, this_shape, strides,
					static_cast<quint8*>(dst->opaque) + vipFlatOffset<false>(dst->strides, dst_start)*dst->dataSize(), other_type, dst_shape, dst->strides);
			else //any other conversion supported by the meta type system
				return vipArrayTransformVoid(static_cast<const T*>(opaque) + vipFlatOffset<false>(strides, this_start), this_type, this_shape, strides,
					static_cast<quint8*>(dst->opaque) + vipFlatOffset<false>(dst->strides, dst_start)*dst->dataSize(), other_type, dst_shape, dst->strides);
		}
		...
		virtual bool importData(const VipNDArrayShape & this_start, const VipNDArrayShape & this_shape, const VipNDArrayHandle * src, const VipNDArrayShape & src_start, const VipNDArrayShape & src_shape)
		{
			...
			if (other_type == this_type)
			{
				return vipArrayTransform(static_cast<const T*>(src->opaque) + vipFlatOffset<false>(src->strides, src_start), src_shape, src->strides,
					static_cast<T*>(opaque) + vipFlatOffset<false>(strides, this_start), this_shape, strides, VipNullTransform());
			}

			if (vipCanConvertStdTypes(other_type, this_type))//conversion of "standard" types (numeric, QString, QByteArray, complex)
				return convert(static_cast<const quint8*>(src->opaque) + vipFlatOffset<false>(src->strides, src_start)*src->dataSize(), other_type, src_shape, src->strides,
					static_cast<T*>(opaque) + vipFlatOffset<false>(strides, this_start), this_type, this_shape, strides);
			else
				return vipArrayTransformVoid(static_cast<const quint8*>(src->opaque) + vipFlatOffset<false>(src->strides, src_start)*src->dataSize(), other_type, src_shape, src->strides,
					static_cast<T*>(opaque) + vipFlatOffset<false>(strides, this_start), this_type, this_shape, strides);
		...
		T * ptr(const VipNDArrayShape & sh) {
			return ((T*)opaque) + vipFlatOffset<false>(strides, sh);
		}

		virtual QVariant toVariant(const VipNDArrayShape & sh) const
		{
			return QVariant::fromValue(*const_cast<StdHandle*>(this)->ptr(sh));
		}
...
namespace detail
{
	...
	struct ViewHandle : public VipNDArrayHandle
	{
		...
		ViewHandle(const ViewHandle & other)
			:handle(other.handle), start(other.start), pointerView(other.pointerView), ptr(other.ptr) {
			opaque = other.opaque;
			strides = other.strides;
			size = other.size;
			shape = other.shape;
		...
		ViewHandle(const ViewHandle & other, const VipNDArrayShape & st, const VipNDArrayShape & sh)
			:handle(other.handle), pointerView(other.pointerView) {
		...
		ViewHandle(const SharedHandle & other, const VipNDArrayShape & start, const VipNDArrayShape & sh)
			: handle(other), start(start), pointerView(false) {
			shape = sh;
		...
		ViewHandle(void *ptr, int type, const VipNDArrayShape & sh, const VipNDArrayShape & _strides)
			:handle(vipCreateArrayHandle(Standard, type, ptr, sh,vip_deleter_type())), pointerView(true)
		{
