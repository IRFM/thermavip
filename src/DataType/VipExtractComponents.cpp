...
struct Clamp {
	...
	T operator()(const T & val) {
		return min ? (val < *min ? *min : (max ? (val > *max ? *max : val) : val))
...
struct ClampArray {
	...
	void operator()(VipNDArrayTypeView<T>  val) {
		vipArrayTransform(val.ptr(), val.shape(), val.strides(),
			val.ptr(), val.shape(), val.strides(), clamp);
	}
};
...
bool VipExtractComponents::SetComponent(const QString & component, const VipNDArray & array)
{
	...
	else
	{
		...
		int type = QMetaType::type(pixel_type.data());
...
{
	QList<VipNDArray> components = GetComponents();
	if(components.size() != 4) return VipNDArray();
...
VipNDArray VipExtractHSLComponents::MergeComponents() const
{
	...
	int * h = (int *) components[0].data();
	int * s = (int *) components[1].data();
	int * l = (int *) components[2].data();
...
VipNDArray VipExtractHSVComponents::MergeComponents()const
{
	...
	const int * h = ( const int *) components[0].data();
	const int * s = ( const int *) components[1].data();
	const int * v = ( const int *) components[2].data();
...
{
	const QList<VipNDArray> components = GetComponents();
	if(components.size() != 5) return VipNDArray();
...
{
	const QList<VipNDArray> components = GetComponents();
	if(components.size() != 1) return VipNDArray();
...
{
	const QList<VipNDArray> components = GetComponents();
	if(components.size() != 2) return VipNDArray();
	...
	VipNDArray res(QMetaType::type("complex_d"),shape);
...
{
	if(strcmp(ar.dataName() ,"complex_f") == 0 || strcmp(ar.dataName() ,"complex_d") == 0)
		return (QStringList()<<"Real"<<"Imag");
	else if(strcmp(ar.dataName(),"QImage") == 0 )
		return (QStringList()<<"Alpha"<<"Red"<<"Green"<<"Blue");
	else if (vipIsMultiNDArray(ar))
