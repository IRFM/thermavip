...
namespace detail
{
	...
	struct SplineInterpolation
	{
		...
		std::complex<T> operator()(
				std::complex<T> y0,std::complex<T> y1,
				std::complex<T> y2,std::complex<T> y3,
		   double mu) const
		{
	...
	struct CatmullRomInterpolation
	{
				   ...
				   double mu) const
		{
			...
			return 0.5 *(  	(2 * y1) +
				(-y0 + y2) * mu +
			(2*y0 - 5*y1 + 4*y2 - y3) * mu2 +
			(-y0 + 3*y1- 3*y2 + y3) * mu2*mu);
		}
	};
	...
	template< class T, class U> struct is_same_interp { enum {value = false}; };
	template< class T> struct is_same_interp<T,T> { enum {value = true}; };
		...
		static void apply(const Src & src, Dst & dst)
		{
			...
			for (int i = 0; i < dst_h; ++i, dy += y_ratio)
			{
				...
				for (int j = 0; j < dst_w; ++j, dx += x_ratio)
				{
					...
					target[j*dst_stride1] = *a*(_1_x_diff)*(_1_y_diff)+*b*(x_diff)*(_1_y_diff)+
						*c*(y_diff)*(_1_x_diff)+*d*(x_diff*y_diff);
				}
			}
		}
	};
	...
	void resize(const Src& src, Dst & dst )
	{
		...
		for(int index = 0; index < src.shapeCount(); ++index)
		{
			...
			for(int pos=0; pos < iter_count; ++pos)
			{
				...
				{
					VipNDArrayType<typename Dst::value_type> * tmp_src = & tmp[!(index%2)];
					ResizeLine::apply(DstLine(tmp_src->ptr(iter.pos),tmp_src->stride(index)),DstLine(tmp_dst.ptr(iter.pos),tmp_dst.stride(index)),src.shape(index),dst.shape(index));
				}
				iter.increment();
			}
		}
