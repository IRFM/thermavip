...
{
	const T value;
	VipFillTransform(const T & val = T()) : value(val) {}
...
#define __NDIM2(expr1,expr2) iter_detail::StaticSize<decltype(expr1)>::value > iter_detail::StaticSize<decltype(expr2)>::value ? \
	iter_detail::StaticSize<decltype(expr1)>::value : iter_detail::StaticSize<decltype(expr2)>::value
...
namespace detail
{
	...
	struct ComputeOffset
	{
		...
		{
			return previous_offset +  ComputeOffset<StridesType,PosType,Dim-1>::
					offset(strides, pos, strides[PosType::static_size-Dim]*pos[PosType::static_size-Dim]);
		}
	};
	...
	struct ComputeOffset<StridesType,PosType, Vip::None>
	{
		...
		{
			switch (pos.size()) {
			case 1: return strides[0] * pos[0];
			case 2: return strides[0] * pos[0]+ strides[1] * pos[1];
			case 3: return strides[0] * pos[0]+ strides[1] * pos[1]+ strides[2] * pos[2];
			default: {
				int res = strides[0] * pos[0];
				for (int i = 1; i < pos.size(); ++i) res += strides[i] * pos[i];
				return res;
			}
			}
		}
	...
	{
		static VIP_ALWAYS_INLINE int offset(const StridesType & strides, const PosType& pos)noexcept {
			if (StridesType::static_size == 1 && Unstrided) return pos[0];
			else return pos[0] * strides[0];
		}
	};
	...
	{
		static VIP_ALWAYS_INLINE int offset(const StridesType & strides, const PosType& pos)noexcept {
			if (StridesType::static_size == 2 && Unstrided) return pos[0] * strides[0] + pos[1];
			else return pos[0] * strides[0] + pos[1] * strides[1];
		}
	};
	...
	{
		static VIP_ALWAYS_INLINE int offset(const StridesType & strides, const PosType& pos)noexcept {
			if (StridesType::static_size == 3 && Unstrided) return pos[0] * strides[0] + pos[1] * strides[1] + pos[2];
			else return pos[0] * strides[0] + pos[1] * strides[1] + pos[2] * strides[2];
		}
	};
}
...
namespace detail
{
	...
	struct ReachEndIncrement<Vip::FirstMajor>
	{
		...
		VIP_ALWAYS_INLINE static bool apply(const Position & pos, const Shape & sh) { return pos[0] == sh[0]; }
	};
	template<>
	struct ReachEndIncrement<Vip::LastMajor>
	{
		template< class Position, class Shape>
		VIP_ALWAYS_INLINE static bool apply(const Position & pos, const Shape & sh) { return pos.back() == sh.back(); }
	};
			...
			if (++pos[0] == sh[0]) {
				...
				for (int i = 1; i < pos.size(); ++i) {
					if (++pos[i] == sh[i]) {
						...
						if (i == pos.size() - 1) pos[i] = sh[i]; //last iteration
					}
					else
						break;
				}
...
//
	...
	struct CIteratorFMajorNoSkip
	{
		...
		CIteratorFMajorNoSkip( const ShapeType & sh , skip_type = 0) :shape(sh)
		{
		...
		{
			setFlatPosition(offset + flatPosition());
		}
	...
	struct CIteratorFMajorSkipDim
	{
		...
		void setFlatPosition(int offset)
		{
			...
			int index = 0;
			for(int i = 0; i < shape.size(); ++i)
				if(i != skip) new_shape[index++] = pos[i];
		...
		bool decrement()
		{
			...
			if(pos[innerDimensionIndex()] == int(-1))
			{
				...
				for(int i = innerDimensionIndex()-1; i>=0; --i) {
					if(i == (int)skip) continue;
					else if(--pos[i] == (int)(-1) ) {pos[i] = shape[i]-1;}
					else break;
				}
			...
			else
			{
				...
				if(rect.first[innerDimensionIndex()] == 0)
				{
					...
					for(; i < shape.size()-1; ++i)
						if(pos[i] < rect.first[i] || pos[i] >= rect.second[i] ) break;
		...
		bool decrement()
		{
			...
			if(pos[innerDimensionIndex()] != int(-1))
			{
				...
				if(rect.first[innerDimensionIndex()] == 0)
				{
					pos[innerDimensionIndex()] = shape.back()-1;
					for(int i = shape.size()-2; i>=0; --i) {
						if(--pos[i] == (int)(-1) ) {pos[i] = shape[i]-1;}
						else break;
					}
	...
	struct CIteratorLMajorNoSkip
	{
		...
		CIteratorLMajorNoSkip( const ShapeType & sh , skip_type = 0) :shape(sh)
		{
			pos.resize(sh->size());
			pos.fill(0);
		...
		bool decrement()
		{
			...
			if(pos[0] == int(-1))
			{
				...
				for(int i=1; i < shape.size(); ++i) {
					if(--pos[i] == (int)(-1) ){
						pos[i] = shape[i]-1;
						if(i==shape.size()-1) pos[i] = 0; //last iteration
					}
					else
						break;
				}
...
public:
	...
	VIP_ALWAYS_INLINE VipNDSubArrayConstIterator(const ShapeType& sh, const ShapeType& st, const TYPE* ptr, int full_size) noexcept //begin
		: shape(sh), strides(st), ptr(ptr), abspos(0), full_size(full_size) //begin
	{
		pos.resize(sh.size());
		pos.fill(0);
	}
	...
	VIP_ALWAYS_INLINE VipNDSubArrayConstIterator(const ShapeType& sh, const ShapeType& st, const TYPE* ptr, int full_size, int pos) noexcept //end
		: shape(sh), strides(st), ptr(ptr), abspos(pos), full_size(full_size)
	{}
	VIP_ALWAYS_INLINE const_reference operator*() const noexcept{
		return ptr[vipFlatOffset<false>(strides, pos)];
	}
	VIP_ALWAYS_INLINE const_pointer operator->() const noexcept {
	...
	VIP_ALWAYS_INLINE VipNDSubArrayConstIterator& operator+=(int diff) noexcept {
		...
		else if (diff == 2) return ++(++(*this));
		else if (diff == 3) return ++(++(++(*this)));
...
public:
	...
	VIP_ALWAYS_INLINE reference operator*()  noexcept {
		return const_cast<reference>(base_type::operator*());
	}
	VIP_ALWAYS_INLINE pointer operator->()  noexcept {
...
public:
	...
	:data(end_ptr),end(end_ptr),data_size(0){}
...
namespace detail
{
	...
	struct VoidToVoid
	{
		int inType;
		int outType;
		VoidToVoid(int i=0, int o=0) : inType(i),outType(o){}
	...
	struct Copy
	{
		static void apply(const T* start, const T* end, U * other, CONVERTER c) {
			detail::stdTransform(start, end, other, c);
		}
	};
	...
	{
		static void apply(const T* start, const T* end, U * other, VipNullTransform) {
			std::copy(start, end, other);
		}
	};
	...
	struct CopyInplace
	{
		static void apply( T* start,  T* end, CONVERTER c) {
			detail::stdInplaceTransform(start, end, c);
		}
	};
	...
	{
		static void apply( T* start,  T* end, VipFillTransform<U> tr) {
			std::fill(start, end, tr.value);
		}
	};
...
bool vipInplaceArrayTransform( INTYPE* in, const VipNDArrayShape & in_shape, const VipNDArrayShape & in_strides, CONVERTER c)
{
	...
	if (in_unstrided)
		...
		detail::stdInplaceTransform(VipNDSubArrayIterator< INTYPE>(in_shape, in_strides, in, size_in),
			VipNDSubArrayIterator< INTYPE>(in_shape, in_strides, in, size_in, size_in),
			 c);
...
bool vipArrayTransform(const INTYPE* in, const VipNDArrayShape & in_shape, const VipNDArrayShape & in_strides,
	OUTTYPE* out, const VipNDArrayShape & out_shape, const VipNDArrayShape & out_strides, CONVERTER c)
{
	bool in_unstrided;
	...
	if (in_unstrided && other_unstrided)
		...
		detail::stdTransform(VipNDSubArrayConstIterator<INTYPE>(in_shape,in_strides,in, size_in),
			VipNDSubArrayConstIterator<INTYPE>(in_shape,in_strides,in, size_in, size_in),
				VipNDSubArrayIterator<OUTTYPE>(out_shape,out_strides,out, size_out), c);
...
/// \param out_shape output array shape
/// \param out_strides output array strides
inline bool vipArrayTransformVoid(const void* in, int in_type, const VipNDArrayShape & in_shape, const VipNDArrayShape & in_strides,
	void* out,  int out_type, const VipNDArrayShape & out_shape, const VipNDArrayShape & out_strides)
{
	bool in_unstrided;
	bool other_unstrided;
	int size_in= vipShapeToSize(in_shape,in_strides, &in_unstrided);
	...
	if(in_unstrided && other_unstrided)
		detail::stdApplyBinary(VipVoidIterator(in_ptr,inSizeOf),VipVoidIterator(static_cast<char*>(in_ptr)+size_out*inSizeOf,inSizeOf),VipVoidIterator(out,outSizeOf), detail::VoidToVoid(in_type,out_type));
	else if(in_unstrided)
		detail::stdApplyBinary(VipVoidIterator(in_ptr,inSizeOf),VipVoidIterator(static_cast<char*>(in_ptr)+size_out*inSizeOf,inSizeOf),VipNDSubArrayIterator<void>(out_shape,out_strides,out,outSizeOf), detail::VoidToVoid(in_type,out_type));
	else if(other_unstrided)
		detail::stdApplyBinary(VipNDSubArrayIterator<void>(in_shape,in_strides,in_ptr,inSizeOf),VipNDSubArrayIterator<void>(in_shape,in_strides,in_ptr,inSizeOf,false),VipVoidIterator(out,outSizeOf), detail::VoidToVoid(in_type,out_type));
	else
		detail::stdApplyBinary(VipNDSubArrayIterator<void>(in_shape,in_strides,in_ptr,inSizeOf),
				...
				VipNDSubArrayIterator<void>(out_shape,out_strides,out,outSizeOf), detail::VoidToVoid(in_type,out_type));
...
namespace iter_detail
{
	...
	VIP_ALWAYS_INLINE  VipHybridVector<int, N> initStart(const VipHybridVector<int, N>&) {
		VipHybridVector<int, N> start; start.fill(0);
		return start;
	}
	template<>
