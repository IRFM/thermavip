...
namespace detail
{
	...
	struct Eval {
		...
		static bool apply(Dst &, const Src &, const OverRoi&) { return false; }
	};
			...
			if ((Src::access_type & Vip::Cwise)&& (Dst::access_type & Vip::Cwise) && size > 4096) { //parallel access
				if ((Src::access_type & Vip::Flat) && dst.isUnstrided() && src.isUnstrided()) {
					...
					if (src.shape().size() == 1) {
						...
						vip_iter_parallel_fmajor(dst.shape(), c)
	...
	struct Eval<DType,false,true, OverRoi>
	{
		...
		static bool apply(Dst & dst, const Src & src, const OverRoi & roi)
		{
			...
			if ((Dst::access_type & Vip::Flat) && (Src::access_type & Vip::Flat) && (OverRoi::access_type & Vip::Flat)
				&& dst.isUnstrided() && src.isUnstrided() && roi.isUnstrided()) {
				const Src s = src;
				for (int i = 0; i < size; ++i)
					if(roi[i])
						ptr[i] = vipCast<dtype>(s[i]);
			...
			if ((Dst::access_type & Vip::Flat) && dst.isUnstrided()) {
				if (src.shape().size() == 1) {
					...
					VipHybridVector<int, 1> p={{0}};
					for (p[0] = 0; p[0] < w; ++p[0])
						if(roi(p)) ptr[p[0]] = vipCast<dtype>(src(p));
					...
					for (p[0] = 0; p[0] < h; ++p[0])
						for (p[1] = 0; p[1] < w; ++p[1], ++i)
							if(roi(p)) ptr[i] = vipCast<dtype>(src(p));
				}
				else if (src.shape().size() == 3) {
					const int z = src.shape()[0];
					const int h = src.shape()[1];
	...
	class SupportFun
	{
		...
		static auto test(int)
			-> decltype(std::declval<F&>()[0], std::true_type());
	...
	{
		template< class Dst>
		static bool apply(Dst & , const Src & ) {return false;}
	};
	template<class Src>
