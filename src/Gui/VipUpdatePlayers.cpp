...
VipUpdateVideoPlayer::VipUpdateVideoPlayer(VipVideoPlayer * player)
	:QObject(player), m_player(player), m_displayMarkerPos(false)
{
	...
	if (VipVideoPlayer * p = m_player)
		if (VipPlotSceneModel * plot_scene = p->plotSceneModel())
		{
			...
			for (int i = 0; i < shapes.size(); ++i)
			{
				...
				//set the texts
				VipText min_t;
				if (m_displayMarkerPos) min_t.setText("<b>" + QString::number(stats.min) + "</b><br>(x:" + QString::number(stats.minPoint.x()) + " , y:" + QString::number(stats.minPoint.y()) + ")");
				else min_t.setText("<b>" + QString::number(stats.min) + "</b>");
				min_t.setTextPen(QColor(Qt::blue));
				min_t.setBackgroundBrush(QBrush(QColor(255, 255, 255, 160)));

				VipText max_t;
				if (m_displayMarkerPos) max_t.setText("<b>" + QString::number(stats.max) + "</b><br>(x:" + QString::number(stats.maxPoint.x()) + " , y:" + QString::number(stats.maxPoint.y()) + ")");
				else max_t.setText("<b>" + QString::number(stats.max) + "</b>");
...
bool VipDrawDistance2Points::sceneEvent(QEvent * event)
{
	...
	{
		QPointF scene_pos = event->type() == QEvent::GraphicsSceneMouseMove ?
			static_cast<QGraphicsSceneMouseEvent*>(event)->scenePos() :
			static_cast<QGraphicsSceneHoverEvent*>(event)->scenePos();
...
void VipUpdatePlotPlayer::distanceCreated(const VipPoint & start, const VipPoint & end)
{
	...
	text += "<b>Euclidean distance: " + (xunit.isEmpty() ? QString() : ("(time in " + xunit + ")")) + ": </b>" +
		QString::number(qSqrt((double)(x2 + y2)));

	QMessageBox mb(QMessageBox::Information, "Distance between points",
		text, QMessageBox::Ok, m_player);
			...
			for (int j = 0; j < vectors.size(); ++j) {
				...
				if (vec.size()) {
					...
					if (m_minmaxPos->isChecked())
						min_t.setText("<b>Min: " + QString::number(vec[min].y()) + "</b><br>(x:" + curve->axes()[0]->scaleDraw()->label( vec[min].x(),VipScaleDiv::MajorTick).text()+")" );
					else
					...
					VipText max_t;
					if (m_minmaxPos->isChecked())
						max_t.setText("<b>Max: " + QString::number(vec[max].y()) + "</b><br>(x:" + curve->axes()[0]->scaleDraw()->label(vec[max].x(), VipScaleDiv::MajorTick).text()+")");
					else
					...
					//set the transform
					QTransform min_tr; min_tr.translate(0, mins[j]->symbol()->size().height() / 2);
					QTransform max_tr; max_tr.translate(0, -maxs[j]->symbol()->size().height() / 2);
