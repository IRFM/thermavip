...
PyBaseHighlighter::PyBaseHighlighter(const QString & name, const QString & type, const QStringList & extensions, QTextDocument *parent )
	:BaseHighlighter(name, type, extensions, parent),m_enable(true)
{
	...
	real = QRegExp("((\\+|-)?([0-9]+)(\\.[0-9]+)?)|((\\+|-)?\\.?[0-9]+)");
	updateRules(); 
}

void PyBaseHighlighter::updateRules()
{
	HighlightingRule rule;
	...
	keywordPatterns << "\\band\\b" << "\\bassert\\b" << "\\bbreak\\b"
		...
		<< "\\bwhile\\b" << "\\byield\\b" << "\\bas\\b" << "\\bself\\b";
	...
	predefinePatterns << "\\bstr\\b" << "\\blen\\b" << "\\bmax\\b"
		<< "\\bmin\\b" << "\\bint\\b" << "\\blong\\b"
		<< "\\bfloat\\b" << "\\bbool\\b" << "\\bstr\\b"
		<< "\\bhelp\\b" << "\\bdir\\b" << "\\bcallable\\b"
		<< "\\blist\\b" << "\\btuple\\b"
		<< "\\bNameError\\b" << "\\bBytesWarning\\b" << "\\bdict\\b"
		<< "\\binput\\b" << "\\boct\\b" << "\\bbin\\b"
		<< "\\bSystemExit\\b" << "\\bStandardError\\b" << "\\bformat\\b"
		<< "\\brepr\\b" << "\\bsorted\\b" << "\\bFalse\\b"
		<< "\\bset\\b" << "\\bbytes\\b" << "\\breduce\\b"
		<< "\\bintern\\b" << "\\bissubclass\\b" << "\\bEllipsis\\b"
		<< "\\bEOFError\\b" << "\\blocals\\b" << "\\bBufferError\\b"
		<< "\\bWarning\\b" << "\\b__package__\\b" << "\\bround\\b"
		<< "\\bRuntimeWarning\\b" << "\\biter\\b" << "\\bcmp\\b"
		<< "\\bslice\\b" << "\\bFloatingPointError\\b" << "\\bsum\\b"
		<< "\\bgetattr\\b" << "\\babs\\b" << "\\bexit\\b"
		<< "\\bTrue\\b" << "\\bFutureWarning\\b" << "\\bImportWarning\\b"
		<< "\\bNone\\b" << "\\bhash\\b" << "\\bReferenceError\\b"
		<< "\\bcredits\\b" << "\\bdel\\b" << "\\bglobals\\b" << "\\brange\\b"
		<< "\\bprint\\b" << "\\bobject\\b";
...
void PyBaseHighlighter::Enable(bool e)
{
	m_enable = e;
}
...
QByteArray PyBaseHighlighter::removeStringsAndComments(const QString & code, int start, int & inside_string_start, QList<QPair<int,int> > & strings, QList<QPair<int, int> > & comments)
{
	...
	for (int i = start; i < ar.size(); ++i) {
		if (ar[i] == '"' || ar[i] == (char)39) {
			...
			if (end < 0) {
				...
				memcpy(m_token, token, 4);
				for (int j = i + (int)strlen(token); j < ar.size(); ++j) copy[j] = ' ';
				return copy; //unbalanced string
			}
			end += (int)strlen(token);
			if (end > ar.size()) end = ar.size();
			//replace string content by spaces
			for (int j = i + (int)strlen(token) ; j < end - (int)strlen(token); ++j) copy[j] = ' ';
...
void PyBaseHighlighter::highlightBlock(const QString &_text)
{
	if( ! m_enable )
		return;
	if (_text.isEmpty()) {
	...
	if (st >= 0) {
		...
		//find string token
		int index = _text.indexOf(m_token);
		if (index >= 0)
		{
			//end of multi line comment
			setFormat(0, index + (int)strlen(m_token), quotationFormat);
			for (int i = 0; i < index; ++i) text[i] = QChar(' '); //remove comment part
			setCurrentBlockState(0);
			if (current != 0 && currentBlock().blockNumber() != document()->blockCount()) {
				//we change the state inside the document, rehighlight all
				rehighlightDelayed();
			}
			start_check_string = index + (int)strlen(m_token);
		}
		else {
			//still inside string
	...
	if (inside_string_start >= 0)
	{
		...
		//replace string
		for (int i = inside_string_start + 1; i < text.size(); ++i) text[i] = QChar(' ');
	...
	if( iclass != -1 )
	{
		...
		if(to < 0) to = text.indexOf(tp, iclass + 5);
		if( to >= 0 )
		{
	...
	*/
	...
	//numbers
	int index = real.indexIn(text);
	while (index >= 0) {
		int length = real.matchedLength();
		setFormat(index, length, numberFormat);
		index = real.indexIn(text, index + length);
	}
}
...
void PyDevScheme::updateEditor(CodeEditor * editor) const
{
...
void PyDarkScheme::updateEditor(CodeEditor * editor) const
{
...
void SpyderDarkScheme::updateEditor(CodeEditor* editor) const
{
...
void PyZenburnScheme::updateEditor(CodeEditor * editor) const
{
...
static int registerFormats()
{
	...
	CodeEditor::registerColorScheme(new PyDevScheme());
	CodeEditor::registerColorScheme(new PyDarkScheme());
	CodeEditor::registerColorScheme(new PyZenburnScheme());
	CodeEditor::registerColorScheme(new SpyderDarkScheme());
	CodeEditor::registerColorScheme(new TextScheme());
	CodeEditor::setStdColorSchemeForType("Python", CodeEditor::colorScheme("Python", "Pydev"));
	CodeEditor::setStdColorSchemeForType("Text", CodeEditor::colorScheme("Text", "Text"));
