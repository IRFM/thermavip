...
struct QuadLineIntersection
{
	...
	:startPoint(pt),midPoint(pt),endPoint(pt)
	{}
...
void VipBoxStyle::computePie(const QPointF & c, const VipPie & pie, double spacing)
{
	...
	{
		QRectF outer_circle(c - QPointF(max_distance_to_center,max_distance_to_center) ,
				c + QPointF(max_distance_to_center,max_distance_to_center) );
		QRectF inner_circle(c - QPointF(min_distance_to_center,min_distance_to_center) ,
				c + QPointF(min_distance_to_center,min_distance_to_center) );
...
bool VipBoxStyle::operator==(const VipBoxStyle & other) const
{
	return other.d_data->brushGradient == d_data->brushGradient &&
		other.d_data->drawLines == d_data->drawLines &&
		other.d_data->pen == d_data->pen &&
		other.d_data->penGradient == d_data->penGradient &&
		other.d_data->brushGradient == d_data->brushGradient &&
		other.d_data->radius == d_data->radius &&
...
{
	return stream <<style.borderPen() <<style.backgroundBrush()<<style.adaptativeGradientBrush() << style.adaptativeGradientPen()
			<<style.borderRadius() <<(int)style.drawLines() <<(int) style.roundedCorners();
}
