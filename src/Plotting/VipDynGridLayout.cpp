...
void VipDynGridLayout::PrivateData::updateLayoutCache()
{
    ...
    for ( QList<QGraphicsLayoutItem*>::iterator it = itemList.begin();
...
QList<QRectF> VipDynGridLayout::layoutItems( const QRectF &rect,
    uint numColumns ) const
{
    ...
    for ( int i = 0; i < itemCount; i++ )
    {
        ...
        QRectF itemGeometry( colX[col], rowY[row],
...
void VipDynGridLayout::layoutGrid( uint numColumns,
    QVector<double>& rowHeight, QVector<double>& colWidth ) const
{
    if ( numColumns <= 0 )
        return;
...
void VipDynGridLayout::stretchGrid( const QRectF &rect,
    uint numColumns, QVector<double>& rowHeight, QVector<double>& colWidth ) const
{
    if ( numColumns == 0 || count() == 0 )
        return;
