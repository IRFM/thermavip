...
void VipPlotMarker::drawLines( QPainter *painter,
    const QList<VipInterval> &scale_rect, const VipCoordinateSystemPtr & m, const VipPoint &pos ) const
{
    if ( d_data->style == NoLine )
        return;

    painter->setPen( (d_data->pen) );
    if ( d_data->style == VipPlotMarker::HLine ||
        d_data->style == VipPlotMarker::Cross )
    {
    ...
    if ( d_data->style == VipPlotMarker::VLine ||
        d_data->style == VipPlotMarker::Cross )
    {
...
void VipPlotMarker::drawLabel( QPainter *painter,
    const QRectF &scale_rect,const VipCoordinateSystemPtr & m, const QPointF &pos ) const
{
	Q_UNUSED(m)
