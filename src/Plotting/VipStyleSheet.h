...
struct VIP_PLOTTING_EXPORT EnumParser : Parser
{
	...
	EnumParser(const QMap<QByteArray, int> & enums) :enums(enums)
	{}
	virtual QVariant parse(const QByteArray & ar);
	virtual QByteArray toString(const QVariant&);
};
...
{
	QMap<QByteArray, int> enums;
	EnumOrParser(const QMap<QByteArray, int> & enums) :enums(enums)
	{}
	virtual QVariant parse(const QByteArray & ar);
	virtual QByteArray toString(const QVariant&);
};
...
/// and associated parssed keywords
...
public:
	...
	/// @param selectors optional class selectors for classes that support it
