...
struct PointMerge
{
	...
	int size() const {
		return buff_size + vector.size();
	}

	const Point last() const {
		return buff_size ? buff[buff_size - 1] : (vector.size() ? vector.last() : Point());
	}
...
static bool isPerfectRightCartesiant(QPainter *painter, const VipCoordinateSystemPtr & m)
{
	...
	QLineF l1(scales.first()->position(scales.first()->scaleDiv().bounds().minValue()),
		scales.first()->position(scales.first()->scaleDiv().bounds().maxValue()));
	QLineF l2(scales.last()->position(scales.last()->scaleDiv().bounds().minValue()),
		scales.last()->position(scales.last()->scaleDiv().bounds().maxValue()));
...
struct Condition
{
	...
	bool isValid(const VipPoint & pt) const {
		...
		switch (comp) {
		case Gr:return v > value;
		case Lr:return v < value;
		case Greq:return v >= value;
		case Lreq:return v <= value;
...
struct MultiCondition
{
	...
	{
		switch (separator) {
		case And: return c1.isValid(pt) && c2.isValid(pt);
		case Or: return c1.isValid(pt) || c2.isValid(pt);
		case None: return c1.isValid(pt);
		}
		return false;
	}
	...
	static MultiCondition * parse(const QString & condition, QString * error)
	{
		...
		QStringList conds;
		if (sep.isEmpty()) conds << condition;
		else conds = condition.split(sep);
		...
		for (int i = 0; i < conds.size(); ++i) {
			...
			else if (parts[1] == ">=") c[i].comp = Condition::Greq;
			else if (parts[1] == "<") c[i].comp = Condition::Lr;
			else if (parts[1] == "<=") c[i].comp = Condition::Lreq;
			else if (parts[1] == "==") c[i].comp = Condition::Eq;
...
void VipPlotCurve::draw(QPainter * painter, const VipCoordinateSystemPtr & m) const
{
	...
	for (int i = 0; i < drawn_polygons.size(); ++i)
	{
		...
		bool drawSelected = isSelected() && selectedPen() != Qt::NoPen && selectedPen().color().alpha() != 0 &&
			boxStyle().borderPen() != Qt::NoPen && boxStyle().borderPen().color().alpha() != 0 && style() != NoCurve //&&
		...
		for (int i = 0; i < points.size(); ++i)
		{
			if (points[i].x() >= x_inter.minValue())
			{
				...
				if (!cartesian || res.isEmpty() || x != last_x) {
					...
					if(base.y() > res[res.size() - 2].y())  res[res.size() - 2].setY(base.y());
					if (tr.y() < res[res.size() - 1].y()) res[res.size() - 1].setY(tr.y());
				}
			}
...
QPolygonF VipPlotCurve::extractEnveloppe(const QPolygonF & points, int factor, double * length) const
{
	...
	QPolygonF upper; upper.reserve(size / factor);
	QPolygonF lower; lower.reserve(size / factor);
...
QPolygonF  VipPlotCurve::drawLines( QPainter *painter, const VipCoordinateSystemPtr & m, const QPolygonF & points, bool draw_selected, bool //continuous
, int index) const
{
	...
	const bool doFill = (boxStyle().backgroundBrush().style() != Qt::NoBrush && (boxStyle().backgroundBrush().color().alpha() > 0))
		&& (!testCurveAttribute(FillMultiCurves) || d_data->full_continuous);
...
void VipPlotCurve::drawSymbols( QPainter *painter, const VipSymbol & symbol, const VipCoordinateSystemPtr & m, const VipPointVector & pts, bool continuous, int ) const
{
	...
	else
	{
		...
		bool draw_selection = isSelected() &&
			(selectedPen() != Qt::NoPen && selectedPen().color().alpha() != 0) &&
			symbol.style() != VipSymbol::Pixmap && symbol.style() != VipSymbol::SvgDocument && symbol.style() != VipSymbol::UserStyle &&
			!computingShape();
...
QRectF VipPlotCurve::drawLegend(QPainter * painter, const QRectF & rect, int) const
{
	...
	if ( d_data->legendAttributes == 0 ||
		(d_data->legendAttributes & VipPlotCurve::LegendShowBrush) )
	{
...
bool VipPlotCurve::setItemProperty(const char* name, const QVariant& value, const QByteArray& index)
{
	if (strcmp(name, "curve-style") == 0) {
		int v = value.toInt();
		if (v < NoCache || v > Dots) return false;
		...
		int v = value.toInt();
		if (v < -1 || v > VipSymbol::Hexagon) return false;
...
inline static bool isPointInside(const QPainterPath & shape, const QRectF & bounding, int shape_coord, const VipPoint & pt)
{
	...
	switch (shape_coord) {
	default:
	case 0: return shape.contains((QPointF)pt);
	case 1: return pt.x() >= bounding.left() && pt.x() <= bounding.right();
	case 2: return pt.y() >= bounding.top() && pt.y() <= bounding.bottom();
	case 3: return pt.x() >= bounding.left() && pt.x() <= bounding.right() && pt.y() >= bounding.top() && pt.y() <= bounding.bottom();
	}
}

QList<VipInterval> VipPlotCurve::dataBoundingRect(
	const VipPointVector & samples,
	...
	int shape_coord)
{
	...
	{
		if (vipIsNan(input[i].x()) || vipIsNan(input[i].y()) || !std::isfinite(input[i].y()) ||
			!isPointInside(shape, bounding,shape_coord, input[i]))
		{
			int len = i - start - 1;
			if (len)
			{
		...
		else
		{
			...
			if (first) {
				...
				if (input[i].x() < topleft.x()) topleft.setX(input[i].x());
				if (input[i].y() < topleft.y()) topleft.setY(input[i].y());

				if (input[i].x() > bottomright.x()) bottomright.setX(input[i].x());
				if (input[i].y() > bottomright.y()) bottomright.setY(input[i].y());
			}
		}
