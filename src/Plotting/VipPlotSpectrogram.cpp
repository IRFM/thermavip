...
public:
    ...
    inline bool intersect( const Point3D vertex[3],
...
inline bool ContourPlane::intersect(
    const Point3D vertex[3], QPointF line[2],
...
inline QPointF ContourPlane::intersection(
    const Point3D& p1, const Point3D &p2 ) const
{
...
ContourLines VipPlotSpectrogram::contourLines(const VipNDArray & array_2D,
    const QRectF &rect, 
    const QList<vip_double> &levels, bool IgnoreAllVerticesOnLevel )
{
    ...
    for (int y = 0; y < value.shape(0) - 1; y++)
    {
        ...
        for (int x = 0; x < value.shape(1) - 1; x++)
        {
            ...
            if ( x == 0 )
            {
                ...
                xy[TopRight].y =( pos.y() );
                xy[TopRight].z =(
                    value(  xy[TopRight].y , xy[TopRight].x)
                );

                xy[BottomRight].x =( pos.x() );
                xy[BottomRight].y =( pos.y() + dy );
                xy[BottomRight].z =(
                    value(  xy[BottomRight].y , xy[BottomRight].x)
                );
            }
