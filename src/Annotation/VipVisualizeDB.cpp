...
struct DBItem : public QTableWidgetItem
{
	...
	DBItem(qint64 event_id, const QString & column, const QVariant & value, Type value_type)
		:eventId(event_id), column(column), value(value), type(value_type)
	{
		switch (type) {
		case String: setText(value.toString()); break;
		case Bool: setText(value.toInt() ? "yes" : "no"); break;
		case Double: setText(value.toString()); break;
		case Time: setText(QString::number(value.toLongLong()/1000000000.0)); break;
		case Date: setText(QDateTime::fromMSecsSinceEpoch(value.toLongLong()/1000000).toString("dd/MM/yyyy\nhh:mm:ss")); break;
		}
		//disable edition
			...
			if (tooltip.isEmpty()) {
				if (VisualizeDB * table = VisualizeDB::fromChild(this->tableWidget())) {
					...
					if (evt.eventId == eventId) {
						...
						_tooltip += QString::number(evt.experiment_id) + " " + evt.camera + " " + evt.device + " " + evt.eventName + " (" +
							    QString::number(evt.confidence) + "/1)";
	...
	:QWidget(parent)
{
	...
	m_data->table->setHorizontalHeaderLabels(QStringList() << "Experiment id" << "Camera" << "Device"<< "Start(s)" << "Duration(s)" << "Type" <<
		 "MaxT(C)" << "Automatic" << "Method" << "Confidence" << "User" << "Comments" << "Name" 
#
	);
	...
	if (watched == m_data->table || watched == m_data->table->viewport())
	{
		...
		else if (evt->type() == QEvent::MouseButtonRelease)
		{
			if (static_cast<QMouseEvent*>(evt)->button() == Qt::RightButton) {
				...
				QSet<int> lines;
				int col = -1;
				for (int i = 0; i < items.size(); ++i) lines.insert(items[i]->row());
				for (int i = 0; i < items.size(); ++i) {
					if (col < 0) col = items[i]->column();
...
QByteArray VisualizeDB::dumpSelection()
{
	...
	for (int i = 0; i < items.size(); ++i)
	{
		...
		if (max_row == -1) max_row = row;
		if (min_col == -1) min_col = col;
		if (max_col == -1) max_col = col;
