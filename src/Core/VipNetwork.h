...
public:
	...
	void	connectToHost(const QString &hostName, quint16 port, QAbstractSocket::OpenMode openMode = QAbstractSocket::ReadWrite, QAbstractSocket::NetworkLayerProtocol protocol = QAbstractSocket::AnyIPProtocol);
...
private Q_SLOTS:
	...
	void	_emit_readyRead() {
		Q_EMIT readyRead();
	}
	void	_emit_connected() {
		Q_EMIT connected();
	}
	void	_emit_disconnected() {
		Q_EMIT disconnected();
	}
	void	_emit_error(QAbstractSocket::SocketError socketError) {
		Q_EMIT error(socketError);
	}
	void	_emit_hostFound(){
		Q_EMIT hostFound();
	}
	void	_emit_proxyAuthenticationRequired(const QNetworkProxy &proxy, QAuthenticator *authenticator) {
		Q_EMIT proxyAuthenticationRequired(proxy, authenticator);
	}
	void	_emit_stateChanged(QAbstractSocket::SocketState socketState) {
		Q_EMIT stateChanged(socketState);
	}
