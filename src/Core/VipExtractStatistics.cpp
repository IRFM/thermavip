...
void VipExtractComponent::apply()
{
	...
	if (in.data().userType() == qMetaTypeId<VipNDArray>())
	{
		...
		if (component.isEmpty())
		{
			...
			if (!component.isEmpty()) propertyAt(0)->setData(component);
		}
...
bool VipSplitAndMerge::setMethod(const QString& method)
{
	...
	if (component_count != m_procList.size())
	{
		...
		for (int i = 0; i < m_procList.size(); ++i)
		{
			...
			connect(m_procList[i], SIGNAL(processingDone(VipProcessingObject*, qint64)),
				this, SLOT(receivedProcessingDone(VipProcessingObject*, qint64)), Qt::DirectConnection);
		}
	}
...
bool VipSplitAndMerge::acceptData(const QVariant & data, const QString & method)
{
	if (method == "Complex Real/Imag" || method == "Complex Amplitude/Argument")
		return data.userType() == qMetaTypeId<complex_f>() || data.userType() == qMetaTypeId<complex_d>() ||
		data.userType() == qMetaTypeId<VipComplexPoint>() || data.userType() == qMetaTypeId<VipComplexPointVector>() ||
		(data.userType() == qMetaTypeId<VipNDArray>() && data.value<VipNDArray>().isComplex());
	else if (method == "Color ARGB" ||
		method == "Color AHSL" ||
		method == "Color AHSV" ||
		method == "Color ACMYK")
...
QStringList VipSplitAndMerge::possibleMethods(const QVariant & data)
{
	...
	else if (data.userType() == qMetaTypeId<complex_f>() ||
		data.userType() == qMetaTypeId<complex_d>() ||
		data.userType() == qMetaTypeId<VipComplexPoint>() ||
...
bool VipSplitAndMerge::acceptInput(int, const QVariant & v)
{
	return v.userType() == qMetaTypeId<VipNDArray>() ||
		v.userType() == qMetaTypeId<complex_f>() ||
		v.userType() == qMetaTypeId<complex_d>() ||
		v.userType() == qMetaTypeId<VipComplexPoint>() ||
