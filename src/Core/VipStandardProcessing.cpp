...
public:
	...
	PrivateData() : isDynamic(false), outputIndex(0), otherPlayerId(0), otherDisplayIndex(0), shouldResizeArray(false){}
};
...
{
	return arch << o.m_data->processing<< o.m_data->parent<<o.m_data->outputIndex<<o.m_data->isDynamic<<
		o.m_data->otherPlayerId<<o.m_data->otherDisplayIndex<<o.m_data->staticData << o.m_data->shouldResizeArray;
}
...
{
	int type = v.userType();
	return type == qMetaTypeId<VipNDArray>() ||
		type == qMetaTypeId<VipPointVector>() ||
		type == qMetaTypeId<VipIntervalSampleVector>();
}
...
void VipClamp::apply()
{
	...
	if (any.data().userType() == qMetaTypeId<VipNDArray>())
	{
		...
		for (int i = 0; i < size; ++i)
		{
			if (min == min && ptr[i] < min) ptr[i] = min;
			else if(max == max && ptr[i] > max) ptr[i] = max;
		}
	...
	else if (any.data().userType() == qMetaTypeId<VipPointVector>())
	{
		...
		for (int i = 0; i < ar.size(); ++i)
		{
			if (min == min && ar[i].y() < min) ar[i].setY( min);
			else if (max == max && ar[i].y() > max) ar[i].setY(max);
		}
	...
	else if (any.data().userType() == qMetaTypeId<VipIntervalSampleVector>())
	{
		...
		for (int i = 0; i < ar.size(); ++i)
		{
			if (min == min && ar[i].value < min) ar[i].value=(min);
			else if (max == max && ar[i].value > max) ar[i].value =(max);
		}
	...
	else
	{
		...
		else if (max == max && val > max) val = (max);
...
, const QVariant & v) const
{
	...
	return type == qMetaTypeId<VipNDArray>() ||
		type == qMetaTypeId<VipPointVector>() ||
		type == qMetaTypeId<VipComplexPointVector>() ||
		type == qMetaTypeId<VipIntervalSampleVector>() ||
		v.canConvert<double>();
}
...
void VipAbs::apply()
{
	...
	if (any.data().userType() == qMetaTypeId<VipNDArray>())
	{
		...
		if(in.canConvert<double>()) {
			...
			int size = ar.size();
			double* ptr = (double*)ar.data();
			for (int i = 0; i < size; ++i) ptr[i] = std::abs(ptr[i]);
			...
			setError("unknown array type", VipProcessingObject::WrongInput);return;
		}
	...
	else if (any.data().userType() == qMetaTypeId<VipPointVector>())
	{
		...
		for (int i = 0; i < ar.size(); ++i)ar[i].setY(vipAbs( ar[i].y()));
		out_data = QVariant::fromValue(ar);
	}
	else if (any.data().userType() == qMetaTypeId<VipIntervalSampleVector>())
	{
		...
		for (int i = 0; i < ar.size(); ++i)ar[i].value = vipAbs(ar[i].value);
		out_data = QVariant::fromValue(ar);
	}
	else if (any.data().userType() == qMetaTypeId<VipComplexPointVector>())
	{
		...
		for (int i = 0; i < ar.size(); ++i)ar[i].setY(std::abs( ar[i].y()));
		out_data = QVariant::fromValue(ar);
	}
	else
	{
...
void VipXOffset::apply()
{
	...
	if (any.data().userType() == qMetaTypeId < VipPointVector>()) {
		...
		if (v.size())
			for (int i = 0; i < v.size(); ++i) v[i].setX(v[i].x() + offset);
...
void FastMedian3x3::apply()
{
	...
	if (this_type == QMetaType::Bool) median_filter((bool*)ar.constData(), (bool*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::Char)  median_filter((char*)ar.constData(), (char*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::SChar) median_filter((signed char*)ar.constData(), (signed char*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::UChar) median_filter((quint8*)ar.constData(), (quint8*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::Short) median_filter((qint16*)ar.constData(), (qint16*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::UShort) median_filter((quint16*)ar.constData(), (quint16*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::Int)median_filter((qint32*)ar.constData(), (qint32*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::UInt) median_filter((quint32*)ar.constData(), (quint32*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::Long) median_filter((long*)ar.constData(), (long*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::ULong) median_filter((unsigned long*)ar.constData(), (unsigned long*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::LongLong) median_filter((qint64*)ar.constData(), (qint64*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::ULongLong) median_filter((quint64*)ar.constData(), (quint64*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::Float)median_filter((float*)ar.constData(), (float*)out.data(), ar.shape(1), ar.shape(0));
	else if (this_type == QMetaType::Double) median_filter((double*)ar.constData(), (double*)out.data(), ar.shape(1), ar.shape(0));
	else {
...
{
public:
	PrivateData() : resample(false), same_object_type(false), same_data_type(false), merge_point_vector(false), resize_array_type(Vip::NoInterpolation)
	{}
...
VipAnyData VipBaseDataFusion::create(const QVariant & data) const
{
	...
	if(inputs().size()) {
		...
		for(int i=0; i < inputs().size(); ++i) {
			...
			if(t != VipInvalidTime)
			{
				if(time == VipInvalidTime) time = t;
				else time = qMax(time,t);
			}
		}
		res.setTime(time);
	}
...
void VipBaseDataFusion::apply()
{
	...
	if (m_data->resample)
	{
		...
		for (int i = 0; i < count; ++i) {
			...
			if (v.userType() == qMetaTypeId<VipNDArray>()) {
				if (sh.isEmpty()) sh = v.value<VipNDArray>().shape();
				else {
		...
		for (int i = 0; i < arrays.size(); ++i) {
			...
			if (! (valid_type && m_data->arrays[i].shape() == sh) ){
				if (dtype) m_data->arrays[i] = VipNDArray(dtype, sh);
				else m_data->arrays[i] = VipNDArray(arrays[i].dataType(), sh);
			}
	...
	if (dtype)
	{
		...
		for (int i = 0; i < count; ++i)
		{
			...
			else if (v.userType() == qMetaTypeId<VipComplexPointVector>())
			{
				...
				if (dtype == qMetaTypeId<complex_f>()) inputs[i].setData(QVariant::fromValue(complex_f(v.toFloat())));
...
template<class T> bool is_min(const T & t1, const T & t2) { return t1 < t2; }
template<> bool is_min(const complex_d & t1, const complex_d & t2){return std::abs(t1) < std::abs(t2);}
template<class T> T _div(const T & v, int div) { return v / div; }
template<> complex_d _div(const complex_d & v, int div) { return complex_d(v.real() / div, v.imag() / div); }
template<class T> T min_val(const T & t1, const T & t2) { return t1 < t2 ? t1 : t2; }
template<class T> T max_val(const T & t1, const T & t2) { return t1 > t2 ? t1 : t2; }
template<> complex_d min_val(const complex_d & t1, const complex_d & t2) { return std::abs(t1) < std::abs(t2) ? t1 : t2; }
template<> complex_d max_val(const complex_d & t1, const complex_d & t2) { return std::abs(t1) > std::abs(t2) ? t1 : t2; }
...
template<class T, bool is_float = std::is_same<T, vip_double>::value || std::is_same<T, float>::value || std::is_same<T, double>::value || std::is_same<T, complex_f>::value || std::is_same<T, complex_d>::value>
struct BIN_OP
{
	template<class OP>
	T operator()(const T & a, const T & b, OP op) const {return op(a, b);}
};
template<class T>
struct BIN_OP<T,true>
{
	...
	T operator()(const T & a, const T & b, OP op) const
	{
		...
		char * _b = (char*)&b;
		char * _o = (char*)&out;
		for (int i = 0; i < (int)sizeof(T); ++i) _o[i] = op(_a[i], _b[i]);
		return out;
	}
};
...
static void __apply_operator_raw(const T * v1, int s1, const T * v2, int s2, T * out, char op)
{
	if (s1 == 1 && s2 == 1) {
		switch (op) {
		case '+': *out = *v1 + *v2; break;
		case '-': *out = *v1 - *v2; break;
		case '*': *out = *v1 * *v2; break;
		case '/': *out = divide( *v1 , *v2); break;
		case '&': *out = BIN_OP<T>()(*v1, *v2,AND()); break;
		case '|': *out = BIN_OP<T>()(*v1, *v2, OR()); break;
		case '^': *out = BIN_OP<T>()(*v1, *v2, XOR()); break;
		}
	}
	else if (s1 == 1) {
		switch (op) {
		case '+': for (int i = 0; i < s2; ++i) out[i] = (*v1) + v2[i]; break;
		case '-': for (int i = 0; i < s2; ++i) out[i] = (*v1) - v2[i]; break;
		case '*': for (int i = 0; i < s2; ++i) out[i] = (*v1) * v2[i]; break;
		case '/': for (int i = 0; i < s2; ++i) out[i] = divide( (*v1) , v2[i] ); break;
		case '&': for (int i = 0; i < s2; ++i) out[i] = BIN_OP<T>()((*v1), v2[i], AND()); break;
		case '|': for (int i = 0; i < s2; ++i) out[i] = BIN_OP<T>()((*v1), v2[i], OR()); break;
		case '^': for (int i = 0; i < s2; ++i) out[i] = BIN_OP<T>()((*v1), v2[i], XOR()); break;
		}
	}
	else if (s2 == 1) {
		switch (op) {
		case '+': for (int i = 0; i < s1; ++i) out[i] = v1[i] + (*v2); break;
		case '-': for (int i = 0; i < s1; ++i) out[i] = v1[i] - (*v2); break;
		case '*': for (int i = 0; i < s1; ++i) out[i] = v1[i] * (*v2); break;
		case '/': for (int i = 0; i < s1; ++i) out[i] = divide( v1[i] , (*v2) ); break;
		case '&': for (int i = 0; i < s1; ++i) out[i] = BIN_OP<T>()(v1[i], (*v2), AND()); break;
		case '|': for (int i = 0; i < s1; ++i) out[i] = BIN_OP<T>()(v1[i], (*v2), OR()); break;
		case '^': for (int i = 0; i < s1; ++i) out[i] = BIN_OP<T>()(v1[i], (*v2), XOR()); break;
		}
	}
	else {
		switch (op) {
		case '+': for (int i = 0; i < s2; ++i) out[i] = v1[i] + v2[i]; break;
		case '-': for (int i = 0; i < s2; ++i) out[i] = v1[i] - v2[i]; break;
		case '*': for (int i = 0; i < s2; ++i) out[i] = v1[i] * v2[i]; break;
		case '/': for (int i = 0; i < s2; ++i) out[i] = divide( v1[i] , v2[i] ); break;
		case '&': for (int i = 0; i < s2; ++i) out[i] = BIN_OP<T>()(v1[i], v2[i], AND()); break;
		case '|': for (int i = 0; i < s2; ++i) out[i] = BIN_OP<T>()(v1[i], v2[i], OR()); break;
		case '^': for (int i = 0; i < s2; ++i) out[i] = BIN_OP<T>()(v1[i], v2[i], XOR()); break;
		}
	}
}
...
static QVariant __apply_operator(const QVariant & v1, const QVariant & v2, char op, VipNDArray & m_buffer)
{
	if (v1.userType() == qMetaTypeId<VipNDArray>() && v2.userType() == qMetaTypeId<VipNDArray>())
	{
		...
		__apply_operator_raw((T*)v1.value<VipNDArray>().constData(), v1.value<VipNDArray>().size(),
			(T*)v2.value<VipNDArray>().constData(), v2.value<VipNDArray>().size(), (T*)m_buffer.data(), op);
	...
	else if (v1.userType() == qMetaTypeId<T>() && v2.userType() == qMetaTypeId<VipNDArray>() )
	{
		...
		T val = v1.value<T>();
		__apply_operator_raw(&val, 1, (T*)v2.value<VipNDArray>().constData(), v2.value<VipNDArray>().size(),
			(T*)m_buffer.data(), op);
		return QVariant::fromValue(m_buffer);
	}
	else if (v1.userType() == qMetaTypeId<VipPointVector>() && v2.userType() == qMetaTypeId<VipPointVector>())
	{
		...
		__apply_operator_raw((const vip_double*)vipExtractYValues(v1.value<VipPointVector>()).constData(), v1.value<VipPointVector>().size(),
			(const vip_double*)vipExtractYValues(v2.value<VipPointVector>()).constData(), v2.value<VipPointVector>().size(), (vip_double*)m_buffer.data(),op);
	...
	else if (v1.userType() == qMetaTypeId<VipPointVector>() && v2.userType() == qMetaTypeId<T>())
	{
		...
		__apply_operator_raw((const vip_double*)vipExtractYValues(v1.value<VipPointVector>()).constData(), v1.value<VipPointVector>().size(),
			&val,1, (vip_double*)m_buffer.data(),op);
		vipSetYValues(samples, m_buffer);
		return QVariant::fromValue(samples);
	}
	...
	else if (v1.userType() == qMetaTypeId<VipComplexPointVector>() && v2.userType() == qMetaTypeId<VipComplexPointVector>())
	{
		...
		__apply_operator_raw((const complex_d*)vipExtractYValues(v1.value<VipComplexPointVector>()).constData(), v1.value<VipComplexPointVector>().size(),
			(const complex_d*)vipExtractYValues(v2.value<VipComplexPointVector>()).constData(), v2.value<VipComplexPointVector>().size(), (complex_d*)m_buffer.data(),op);
		vipSetYValues(samples, m_buffer);
		return QVariant::fromValue(samples);
	}
	else if (v1.userType() == qMetaTypeId<VipComplexPointVector>() && v2.userType() == qMetaTypeId<complex_d>())
	{
		...
		__apply_operator_raw((const complex_d*)vipExtractYValues(v1.value<VipComplexPointVector>()).constData(), v1.value<VipComplexPointVector>().size(),
			&val, 1, (complex_d*)m_buffer.data(),op);
		vipSetYValues(samples, m_buffer);
		return QVariant::fromValue(samples);
	}
	else if (v1.userType() == qMetaTypeId<complex_d>() && v2.userType() == qMetaTypeId<VipComplexPointVector>())
	{
		...
		__apply_operator_raw(&val, 1, (const complex_d*)vipExtractYValues(v2.value<VipComplexPointVector>()).constData(), v2.value<VipComplexPointVector>().size(), (complex_d*)m_buffer.data(),op);
		vipSetYValues(samples, m_buffer);
		return QVariant::fromValue(samples);
	}
	...
	{
		switch (op) {
		case '+': return QVariant::fromValue(v1.value<T>() + v2.value<T>());
		case '-': return QVariant::fromValue(v1.value<T>() - v2.value<T>());
		case '*': return QVariant::fromValue(v1.value<T>() * v2.value<T>());
		case '/': return QVariant::fromValue(v1.value<T>() / v2.value<T>());
		case '&': return QVariant::fromValue(BIN_OP<T>()(v1.value<T>(), v2.value<T>(), AND()));
		case '|': return QVariant::fromValue(BIN_OP<T>()(v1.value<T>(), v2.value<T>(), OR()));
		case '^': return QVariant::fromValue(BIN_OP<T>()(v1.value<T>(), v2.value<T>(), XOR()));
		}
	}
	return QVariant();
}
...
void VipTimeDifference::apply()
{
	...
	double factor = 1;
	if (unit == "us") factor = 1.0 / 1000.0;
	else if (unit == "ms") factor = 1.0 / 1000000.0;
	else if (unit == "s") factor = 1.0 / 1000000000.0;
