...
It is called:
     ...
     int *BuildTriangleIndexList ( void *, float, int *, int , int , int * );

It takes 6 arguments:

int *BuildTriangleIndexList (
...
integers instead, you only need to do a search and replace to change int to int.
The few references to int are from me, not from Clarkson.

The function does not need to be initialized or closed down.
You are likely to want 4 variables to use it:

  int *triangleIndexList;   <- OUTPUT, this does not need initialization
...
and in the statement that draws the triangles. For example:
  ...
  bd.ByteWidth = sizeof( int ) * numTriangleVertices;    <-- numTriangleVertices from the function
...
simplex* new_block_simplex(int make_blocks)
{
	...
	for (i = 0; i < num_simplex_blocks; i++)
		free(simplex_block_table[i]);
...
basis_s* new_block_basis_s(int make_blocks)
{
	...
	for (i = 0; i < num_basis_s_blocks; i++)
		free(basis_s_block_table[i]);
...
static double sc(basis_s* v, simplex* s, int k, int j)
{
	...
	if (j < 10) {
		...
		if (j == 0) {
			...
			for (i = k - 1, sni = s->neigh + k - 1; i > 0; i--, sni--) {
				snib = sni->basis;
				sc_Sb += snib->sqb;
				sc_ldetbound += std::logb(snib->sqb) / 2 + 1;
	...
	temp = v->sqb;
	if (temp)
		temp = std::logb(temp) * 0.5;
	if (sc_ldetbound - v->lscale + temp + 1 < 0) {
		return 0;
	}
	else {
		sc_lscale = (int)floor(std::logb(2 * sc_Sb / (v->sqb + v->sqa * B_ERR_MIN))) / 2;
...
static int out_of_flat(simplex* root, point p)
{
	...
	get_basis_sede(root);
	if (root->neigh[0].vert == hull_infinity)
		return 1;
	reduce(&out_of_flat_p_neigh.basis, p, root, cdim);
	if (out_of_flat_p_neigh.basis->sqa != 0)
		return 1;
	cdim--;
	return 0;
}
...
static int sees(site p, simplex* s)
{
	...
	tt = s->neigh[0].vert;
	if (p == hull_infinity)
		memcpy(sees_b, infinity_basis, basis_s_size);
...
// ----------------------------------------------------------------
static simplex* facet_test(simplex* s, void* /*dummy*/)
{
	return (!s->peak.vert) ? s : nullptr;
}
// -------------------------------------------
static int hullt(simplex* /*s*/, int i, void* /*dummy*/)
{
	return i > -1;
}
// -------------------------------------------
static int truet(simplex* /*s*/, int /*i*/, void* /*dum*/)
{
	return 1;
}
// -------------------------------------------
...
static simplex* extend_simplices(simplex* s)
{
	...
	if (s->visit == pnum)
		return s->peak.vert ? s->neigh[ocdim].simp : s;
	s->visit = pnum;
...
#define pops(x) x = *(search_st + --tms);
	...
	while (tms) {
		if (tms > search_ss)
			search_st = (simplex**)realloc(search_st, ((search_ss += search_ss) + MAXDIM + 1) * sizeof(simplex*));
		pops(s);
		if (s->visit == pnum)
			continue;
		s->visit = pnum;
...
static site get_next_site(void)
{
	...
	if (numPointsProcessed >= totalInputPoints) {
		...
		ptrToOutputList = (int*)malloc((maxOutputEntries + 1) * sizeof(int));
		currenOutputIndex = 0;
		return 0;
	}
...
static simplex* facets_print(simplex* s, void* /*p*/)
{
	...
	triangleList_out(site_numm(v[0]), site_numm(v[1]), site_numm(v[2]), (pdim == 3) ? site_numm(v[3]) : 0);
	return nullptr;
}
...
{
	return (((b[0] - a[0]) * (b[1] + a[1]) + (c[0] - b[0]) * (c[1] + b[1]) + (a[0] - c[0]) * (a[1] + c[1])) > 0);
}
// ---------------------------------------------------------------------------
	...
	if (v0 >= 0 && v1 >= 0 && v2 >= 0 && v3 >= 0) {
		...
		if (triangleDirection) {
			...
			if (((triangleDirection > 0) && !isCW) || // if user wants CW triangles, but it is not CW
				...
				ptrToOutputList[currenOutputIndex++] = (int)v2;
				ptrToOutputList[currenOutputIndex++] = (int)v1;
				ptrToOutputList[currenOutputIndex++] = (int)v0;
				return;
			}
		}
		ptrToOutputList[currenOutputIndex++] = (int)v0;
