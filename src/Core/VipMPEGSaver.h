#ifndef VIP_MPEG_SAVER_H
#define VIP_MPEG_SAVER_H

#include "VipIODevice.h"

/// @brief Additional parameters for VipMPEGSaver
struct VipMPEGIODeviceHandler
{
	int width{ 0 };
	int height{ 0 };
	int fps{ 25 };
...
///
class VIP_CORE_EXPORT VipMPEGSaver : public VipIODevice
{
	Q_OBJECT
...
public:
	...
	virtual QString fileFilters() const { return "Video file (*.mp4 *.mpg *.mpeg *.avi *.wmv *.gif *.mov *.mkv)"; }
	virtual void close();
