...
{
    "=style",       QT_TRANSLATE_NOOP("VipCommandOptions", "sets the application GUI style"),
    "=stylesheet",  QT_TRANSLATE_NOOP("VipCommandOptions", "sets the application stylesheet"),
    "=session",     QT_TRANSLATE_NOOP("VipCommandOptions", "restores the application from an earlier session"),
    "widgetcount",  QT_TRANSLATE_NOOP("VipCommandOptions", "displays debugging information about widgets"),
    "reverse",      QT_TRANSLATE_NOOP("VipCommandOptions", "use right-to-left layout"),
#ifdef QT_DEBUG
    "nograb",       QT_TRANSLATE_NOOP("VipCommandOptions", "never grab the mouse or keyboard"),
#endif
#if defined(QT_DEBUG) && defined(Q_WS_X11)
    "dograb",       QT_TRANSLATE_NOOP("VipCommandOptions", "grab the mouse/keyboard even in a debugger"),
    "sync",         QT_TRANSLATE_NOOP("VipCommandOptions", "run in synchronous mode for debugging"),
#endif
#ifdef Q_WS_WIN
    "direct3d",     QT_TRANSLATE_NOOP("VipCommandOptions", "use Direct3D by default"),
#endif
#ifdef Q_WS_X11
    "=display",     QT_TRANSLATE_NOOP("VipCommandOptions", "sets the X11 display"),
    "=geometry",    QT_TRANSLATE_NOOP("VipCommandOptions", "sets the geometry of the first window"),
    "=font",        "",
    "=fn",          QT_TRANSLATE_NOOP("VipCommandOptions", "sets the default font"),
    "=background",  "",
    "=bg",          QT_TRANSLATE_NOOP("VipCommandOptions", "sets the default background color"),
    "=foreground",  "",
    "=fg",          QT_TRANSLATE_NOOP("VipCommandOptions", "sets the default foreground color"),
    "=button",      "",
    "=btn",         QT_TRANSLATE_NOOP("VipCommandOptions", "sets the default button color"),
    "=name",        QT_TRANSLATE_NOOP("VipCommandOptions", "sets the application name"),
    "=title",       QT_TRANSLATE_NOOP("VipCommandOptions", "sets the application title"),
    "=visual",      QT_TRANSLATE_NOOP("VipCommandOptions", "sets the X11 visual type"),
    "=ncols",       QT_TRANSLATE_NOOP("VipCommandOptions", "limit the number of colors on an 8-bit display"),
    "cmap",         QT_TRANSLATE_NOOP("VipCommandOptions", "use a private color map"),
    "=im",          QT_TRANSLATE_NOOP("VipCommandOptions", "sets the input method server"),
    "noxim",        QT_TRANSLATE_NOOP("VipCommandOptions", "disable the X VipInput Method"),
    "=inputstyle",  QT_TRANSLATE_NOOP("VipCommandOptions", "sets the style used by the input method"),
#endif
    0,              0
};
...
bool VipCommandOptions::alias(const QString& from, const QString& to)
{
    ...
    if (!option) return false; // findOption outputs the warning
...
void CommandOptionsPrivate::parse(const QStringList& params)
{
    ...
    while (pos < ct)
    {
        ...
        if (!endFlags && ((flagStyle == VipCommandOptions::Slash && param[0] == '/') ||
                          (flagStyle != VipCommandOptions::Slash && param[0] == '-')))
        {
            // tagged argument
            if (param.length() == 1)
            {
           ...
           // notUnrecognized = false;
            ...
            else
            {
                ...
                else
                {
                    ...
                    else if ((paramStyle & VipCommandOptions::Space)
                            && (option->paramType & VipCommandOptions::ValueOptional)
                            && !hasEquals)
                    {
                        if (pos < params.count())
                        {
